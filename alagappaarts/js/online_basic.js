// update base url ( http://localhost/savsoftstudent/ ). include slash at end
//var base_url = "http://localhost/projects/test/alagappa/dance/";

var getUrl = window.location;
//var baseUrl = getUrl .protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1]+ "/" + getUrl.pathname.split('/')[2];
var baseUrl = getUrl .protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1];

var base_url = baseUrl+"/dance/";
...
var cq=document.getElementById('current_question').value;
	var aurl=base_url+"student/update_answer/"+cq+"/"+oid+"";
	$.ajax({
		url: aurl
...
var questName = $.trim(qname).replace(/\s+/g, '-');
	$.ajax({
		url: base_url + "qbank/select_questions/"+quid+"/"+questName+"/"+limi+"/"+cid,
		success: function(data){ 
		$("#qbank").html(data);
...
function closeqselection(quid){
		...
		window.location=base_url+"student/edit_student/"+quid;
}

function addquestion(quid,qid){
	$.ajax({
		url: base_url + "student/add_question/"+quid+"/"+qid,
		success: function(data){
		},
		error: function(xhr,status,strErr){
			//alert(status);
...
var check_color_qus="0";
	...
	for(var x=0; x<=noq; x++){ 
			...
			var qid="ques"+id;
			document.getElementById(qid).style.display="inline-table";
			document.getElementById(qid).style.visibility="visible";
			document.getElementById('current_question').value=id;
			var rurl=base_url+"student/update_time/"+cq+"/"+qtime+"";
			$.ajax({
				url: rurl
...
function update_curr_ans(key,q_type,qid){
	...
	if(q_type=="1")
	{
		...
		cq=cq-1;
					//alert(cq);
					var aurl=base_url+"student/update_answer/"+cq+"/"+vals+"";
					$.ajax({
						url: aurl
	...
	if(q_type=="2" || q_type=="3")
	{
			...
			var cq=document.getElementById('current_question').value;
					...
					$.ajax({
						...
						//var aurl=base_url+"student/update_fillups/"+qid+"/"+optn_value_user+"";
							var formData = {q_id:qid,optn_value_user:optn_value_user,q_type:q_type};
								$.ajax({
									type: "POST",
									data : formData,
									url: base_url + "student/update_fillups/",
									success: function(data){
