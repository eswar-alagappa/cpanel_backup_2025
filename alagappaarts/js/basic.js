...
var base_url = baseUrl+"/admin/";

//alert( 'base_url-> '+base_url )
//var base_url = "http://alagappa.sanjaytechnologies.net/alagappa/dance/admin/";
...
$(document).ready(function(){
	...
	var cid = $('#cid').val();
	$.ajax({
		url: base_url + "qbank/get_level_question/" + did + "/" + cid,
...
function questionselection(quid,qname,limi,cid,course_id){
...
$("#qbank").html(vall);*/
var questName = $.trim(qname).replace(/\s+/g, '-');
	$.ajax({
		url: base_url + "qbank/select_questions/"+quid+"/"+questName+"/"+limi+"/"+cid+"/"+course_id,
		success: function(data){ 
		$("#qbank").html(data);
...
var formData = {search_type:search_type,search:searchval};
	$.ajax({
		 type: "POST",
		 data : formData,
			url: base_url + "qbank/select_questions/"+quid+"/"+qname+"/"+limi,
		success: function(data){
		$("#qbank").html(data);
...
function closeqselection(quid){
		...
		window.location=base_url+"quiz/edit_quiz/"+quid;
}

function addquestion(quid,qid){
	$.ajax({
		url: base_url + "quiz/add_question/"+quid+"/"+qid,
		success: function(data){
		},
		error: function(xhr,status,strErr){
			//alert(status);
...
//                    alert(data)
		...
		alert("Unable to add score please check your value"); document.getElementById(div_id).focus(); document.getElementById(div_id).value='';return false;
	}
...
var check_color_qus="0";
	...
	for(var x=0; x<=noq; x++){ 
			...
			var rurl=base_url+"quiz/update_time/"+cq+"/"+qtime+"";
			$.ajax({
				url: rurl
...
function update_curr_ans(key,q_type,qid){
	...
	if(q_type=="1")
	{
		...
		cq=cq-1;
					//alert(cq);
					var aurl=base_url+"quiz/update_answer/"+cq+"/"+vals+"";
					$.ajax({
						url: aurl
	...
	if(q_type=="2" || q_type=="3")
	{
			...
			var cq=document.getElementById('current_question').value;
					...
					$.ajax({
						...
						//var aurl=base_url+"quiz/update_fillups/"+qid+"/"+optn_value_user+"";
							var formData = {q_id:qid,optn_value_user:optn_value_user,q_type:q_type};
								$.ajax({
									type: "POST",
									data : formData,
									url: base_url + "quiz/update_fillups/",
									success: function(data){
