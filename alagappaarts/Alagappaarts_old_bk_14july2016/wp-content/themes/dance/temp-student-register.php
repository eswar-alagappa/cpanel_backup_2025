...
/*$hashrealstr=trim(base64_encode(strtoupper(trim($_POST['defaultReal']))));

echo $hashrealstr== $_POST['defaultRealHash'];
echo '<br/>';
echo $hashrealstr.' == '. $_POST['defaultRealHash'];
exit;*/
...
$mobile=$_REQUEST['txtMobile'];$email_id=$_REQUEST['txtEmail'];$address=$_REQUEST['txtAddress'];$city=$_REQUEST['txtCity'];$state=$_REQUEST['txtState'];
		...
		if($count){
			...
			$programcenter = $wpdb->get_results($sql);
			
			$my_file =$_FILES['flePhoto']['name'];
    
                       $target_path = "/home/appaarts/public_html/userpanel/uploads/student/";
                        	//$target_path = "http://alagappaarts.com/userpanel/uploads/student/";
			...
			if($wpinsertquery)
			{
				...
				$amessage = "Dear APAA Team,<br/><br/><strong>".$firstname." ".$lastname."</strong> has requested for enrolling under the <strong>".$programcenter[0]->programname."</strong> through the dance center - <strong>".$programcenter[0]->centername."</strong>.". "The student details are,<br/><br/>";
...
<div class="fl"><img src="<?php bloginfo('stylesheet_directory'); ?>/images/register-title-bg-left.png" width="5" height="44" /></div><div class="registerFormTitle">To Get Admissions, Please fill in the form given below.</div><div class="fr"><img src="<?php bloginfo('stylesheet_directory'); ?>/images/register-title-bg-right.png" width="5" height="44" /></div>
...
<li class="studentdate"><label>D.O.B : </label><input name="txtdob" type="text" class="datetxtbox"  id="datepicker"/></li>
