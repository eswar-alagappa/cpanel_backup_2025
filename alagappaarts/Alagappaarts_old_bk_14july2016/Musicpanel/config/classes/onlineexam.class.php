...
class onlineexam
{
 ...
 ";
	...
	function arrayMultipleChoice($multiplechoices)
	{
		...
		{
			$arrChoices['choice'.$i] = $option['choice'];
			$i++;
		}
		return $arrChoices;
	}
	...
	function insertExamDetails($arrexamdetails)
	{
		...
		$answerquery = "select answers  
				from online_exam_details
			  where assign_exam_id 	  = {$arrexamdetails[assign_exam_id]} and  question_id =  {$arrexamdetails[question_id]}";
			  $answer = $DB->getArray($answerquery);if(!$answer){
 ...
 ";
	...
	function getNextSetofQuestion($arrayfin){
		...
		foreach ($matchthefollowingquestions as $matchthefollowing){
			...
			$matchthefollowinganswerstodisplay[$j][question] = $matchthefollowing[question];
			...
			$matchthefollowinganswerstodisplay[$j][answer] = $matchthefollowinganswers[$j][answer];
	...
	function getQuestiontoAttend($questionid,$position,$assignedid){
		
		global $DB;
		 
		
		$getquestion = "select qm.id as questionid, qm.question, qm.answer, qm.question_type_id, qt.name as question_type_name, km.value as questiontype from questionmaster qm join questiontype qt on qt.id = qm.question_type_id join keywordmaster km on km.id = qt.controller_id where qm.status = 'Y' and qm.id = '$questionid'";
		$execuetquestion = $DB -> getArray($getquestion);
		//echo "<pre>";
		//print_r($execuetquestion );
		$insertexamdetails = "insert into online_exam_details (assign_exam_id,question_id,question_type_id,attendstatus,position) values ('{$assignedid}','{$questionid}','{$execuetquestion[0][question_type_id]}','N','{$position}')";
		//echo $insertexamdetails; exit;
		$execinsertexamdetails = $DB -> Execute($insertexamdetails);
		 

		
		
		return 	$execinsertexamdetails ;	
		
	}
	function getcurrentQuestiondetails($assignexamid){
		global $DB;
		$getcurrentquestionquery = "select oed.id, oed.question_id ,oed.question_type_id ,oed.answers ,oed.matchanswer,oed.mark,oed.attendstatus from online_exam_details oed  where oed.attendstatus = 'N' and  oed.assign_exam_id =  $assignexamid order by oed.id limit 0,1 ";
		$currentquestionarray = $DB -> getArray($getcurrentquestionquery);
	
		return $currentquestionarray;
	}
	function getnextQuestiondetails($assignexamid){
		global $DB;
		$getcurrentquestionquery = "select 
oed.id, 
oed.question_id ,
oed.question_type_id ,
oed.answers ,
oed.matchanswer,
oed.mark,
oed.attendstatus ,
oed.position ,
(select value from  keywordmaster   where id =   qt.controller_id ) as  questiontype ,
qt.name as question_type_name,
(select question from  questionmaster   where id =   oed.question_id) as  question 
from online_exam_details oed join questiontype qt on  oed.question_type_id  = qt.id where  oed.attendstatus = 'N' and   oed.assign_exam_id =  $assignexamid  order by oed.id limit 0,1 ";
		$currentquestionarray = $DB -> getArray($getcurrentquestionquery);
	    if($currentquestionarray[0]['question_type_id'] == 1){
			$getchoices = "select choice,answerindex from multiple_choice_answer  where  question_id = ".$currentquestionarray[0]['question_id'];
			$execgetchoices = $DB -> getArray($getchoices);
			//  echo "<pre>";
			//  print_r($execgetchoices);
			$currentquestionarray[0]['choices'] = $execgetchoices ;
			//$arrayOld = $this -> querytoArrayMultipleChoice($execgetquestion);
			}
			
			// print_r($currentquestionarray); exit;
		return $currentquestionarray;
	}
	function updateExamDetails($arrexamdetails)
	{
		global $DB;
		
				 
		$insertexamdetails = "update  online_exam_details set answers = '{$arrexamdetails[answers]}',mark =  '{$arrexamdetails[mark]}' ,matchanswer = '{$arrexamdetails[matchanswer]}' , attendstatus = 'Y' where id = {$arrexamdetails[id]}";
	...
	function getCurrentpos($currentquestion,$getTestdetailid){
		global $DB;
		$getquestionquery = "select  oed.id,oed.question_id from online_exam_details oed   where oed.assign_exam_id =  {$getTestdetailid} ";
		
		$questionarray = $DB -> getArray($getquestionquery);
		echo $currentquestion;
		//echo "<pre>";
		//print_r($questionarray);
		foreach($questionarray  as $key =>$question){
			if($question['question_id'] ==$currentquestion )
			$questCount = $key ;
		}
		return $questCount;
	}
	function getNextSetofMatchQuestion($getTestdetailid){
		global $DB;
		$i=0;
		$getquestionquery = "select  oed.id,oed.question_id,oed.question_type_id,(SELECT question
FROM questionmaster  where id =oed.question_id) as question,(SELECT answer
FROM questionmaster  where id =oed.question_id) as answer,(SELECT value 
FROM  keywordmaster  where id =qt.controller_id ) as questiontype from online_exam_details oed 
join  questiontype qt on oed.question_type_id = qt.id 
		  where oed.assign_exam_id =  {$getTestdetailid} and oed.question_type_id=3 ";
		  
		// echo $getquestionquery; exit;
		$matchthefollowingquestions = $DB -> getArray($getquestionquery);
		
		$j=0;$k=1;
		foreach ($matchthefollowingquestions as $matchthefollowing ){
				$matchthefollowingquestions[$j][answercode] =$k;
				$matchthefollowingcorrectanswer[$matchthefollowing[question_id]] =$k;
				$matchthefollowinganswers[$j][answer] = $matchthefollowing[answer];
				$matchthefollowinganswers[$j][correctquestionid] = $matchthefollowing[question_id];
				$questiontypeid =$matchthefollowing[question_type_id];
				$j++;$k++;
		}
		...
		$j=0;
		foreach ($matchthefollowingquestions as $matchthefollowing){
			$matchthefollowinganswerstodisplay[$j][id] = $matchthefollowing[question_id];
			$matchthefollowinganswerstodisplay[$j][question] = $matchthefollowing[question];
			$matchthefollowinganswerstodisplay[$j][question_type_id] = $matchthefollowing[question_type_id];
			$matchthefollowinganswerstodisplay[$j][questiontype] = $matchthefollowing[questiontype];
			$matchthefollowinganswerstodisplay[$j][answer] = $matchthefollowinganswers[$j][answer];
	 ...
	 function getTotalattendcount($getTestdetailid){
		 global $DB;
		  $getquestioncountquery = "select count(*)  as countofattendquestion from online_exam_details where assign_exam_id = '{$getTestdetailid}' and  attendstatus = 'Y'";
		 // echo $getquestioncountquery;
		$totalattendcount = $DB -> getArray($getquestioncountquery);
		return  $totalattendcount[0]['countofattendquestion'] ;
	}
