...
class exammaster
{
		...
		global $DB;
  		...
  		foreach ($arrassginExams['assignExam'] as $arrassginExam){
			...
			$check_exam ="select count(id) as countid from assign_exam where student_id='{$arrassginExams[student_id]}' AND course_id='{$arrassginExam[ddlcourse]}'";
					
		$alreadyexist  = $DB->getArray($check_exam);
		//echo $alreadyexist[0][countid]; 
	
if($alreadyexist[0][countid]==0)
						{
		...
		function getCourseforonlineexammark($studentid){
			global $DB;
		$getcourseforStudent ="select  ae.course_id as id,	cm.code as code ,ae.id as examid 
...
join  questiontype qt on  qt.id= ce.questiontype_id where ce.course_id = {$studentarray[course_id]}  and  qt.controller_id != (select id from  keywordmaster where code ='controller' and value='subjective' )";
		...
		join  questiontype qt on  qt.id= ce.questiontype_id where ce.course_id = {$studentarray[course_id]}  and  qt.controller_id = (select id from  keywordmaster where code ='controller' and value='subjective' ) ";
			...
			function getsubjectiveanswer($studentarray){
					global $DB;
		$getsubjectiveanswer ="select  oed.answers  ,oed.id, oed.question_type_id ,(SELECT qt.marks_per_question FROM questiontype qt WHERE qt.id=oed.question_type_id) as marks_per_question, (select qm.question  from questionmaster  qm where qm.id= oed.question_id 	 ) as question,oed.question_id,oed.comments  ,
		oed.mark
					from assign_exam ae 
...
and ae.result != (select id from keywordmaster where code ='result' and value='Admin reassign') ORDER BY ae.id DESC";
		...
		global $DB;
		$getExternalMark = "select em.id as examid, em.course_id ,cm.code ,cm.regulation_id ,km.value as regulation,km1.value as result,km2.value as grade, em.mark as total_mark, em.video1 as video1, em.video2 as video2, em.video3 as video3
        from external_mark em join coursemaster cm  on  cm.id=em.course_id  
		join keywordmaster km on km.id= cm.regulation_id 
...
$ch = curl_init();
    ...
    curl_setopt($ch, CURLOPT_URL, "http://inqtechnologies.com.in/alagappaarts/mailing1.php");
...
$ch = curl_init();
    ...
    curl_setopt($ch, CURLOPT_HEADER, 0);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_URL, "http://inqtechnologies.com/alagappaarts/mailing1.php");
curl_setopt($ch, CURLOPT_POST, true);
curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
    $data1 = curl_exec($ch);
    curl_close($ch);
		...
		case 1 : 
		$getonemarkanswer
		 ="select oed.id, oed.question_type_id ,(SELECT qtp.marks_per_question FROM questiontype qtp WHERE qtp.id=oed.question_type_id) as mark_per_qtn,(select qm.question  from questionmaster qm where qm.id= oed.question_id ) as question,oed.question_id,oed.comments  ,oed.mark  , mca.choice  as  answers from assign_exam ae 
		join  online_exam_details oed on  ae.id= oed.assign_exam_id 
		...
		and ae.course_id={$studentarray[course_id]} and oed.assign_exam_id ={$studentarray[assign_exam_id]}";
		 break ;
		 default : 
		 $getonemarkanswer ="select  oed.answers  ,oed.id, oed.question_type_id , (SELECT qtp.marks_per_question FROM questiontype qtp WHERE qtp.id=oed.question_type_id) as mark_per_qtn, (select qm.question  from questionmaster  qm where qm.id= oed.question_id 	 ) as question,oed.question_id,oed.comments  ,
		oed.mark
					from assign_exam ae 
		join  online_exam_details oed on  ae.id= oed.assign_exam_id 
		where oed.question_type_id 	={$studentarray[questiontype_id]}    and ae.student_id 	= {$studentarray[student_id]}  and  ae.course_id	={$studentarray[course_id]} and oed.assign_exam_id ={$studentarray[assign_exam_id]} ";
