...
//echo $queryloginDetails;exit;
	...
	function changepassword($arrstudentpassworddetails){
			...
			$checkpwd="select password from loginmaster where user_id='{$arrstudentpassworddetails[user_id]}' and role_id= (select id from rolemaster where rolename ='Student' and  status ='Y')";
			$excuteSelect = $DB->getArray($checkpwd);
			if($excuteSelect){
			$update = "update loginmaster set password='{$arrstudentpassworddetails[password]}'  , modified_by ='{$arrstudentpassworddetails[loginid]}' ,modified_on = '$mysql_datetime' , ip_address ='{$_SERVER['REMOTE_ADDR']}'  where user_id={$arrstudentpassworddetails[user_id]} and role_id= (select id from rolemaster where rolename ='Student' and  status ='Y')";
	...
	function changecentrepassword($arrcenterpassworddetails){
         ...
         function refcode_check($refcode)
	{
		global $DB;
		$refname="select count(id) as countid from loginmaster where username='".$refcode."'";
		$ref_name= $DB -> getArray($refname);
		return $ref_name;
	}
