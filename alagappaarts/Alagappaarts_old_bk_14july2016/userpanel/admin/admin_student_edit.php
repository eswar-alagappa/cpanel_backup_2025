...
{
 var selecteddob = $("#datepicker").val();
		var curyear = (new Date).getFullYear();
		var getyear =selecteddob.substring(selecteddob.lastIndexOf('/'));
		var selectedyear = getyear.replace('/','');
		var age = parseInt(curyear) - parseInt(selectedyear);
		if(selecteddob==''){
			$('#txtAge').val();
			}else{
		$('#txtAge').val(age);
    ...
    <div class="registrationContent">
	 ...
	 $directorDOB =date('m/d/Y', strtotime($dob[1].'/'.$dob[2].'/'.$dob[0])); 
       ...
       <li><label> Age :<strong class="star">*</strong></label>
		<input name="txtAge" id="txtAge" type="text" readonly="readonly" value="<?php echo $studentdetail->fields[age]; ?>" maxlength="3" />  </li>
...
<input type="text" name="txtEnrollmentid" id="txtEnrollmentid" value="<?php echo $studentdetail->fields[enrollment_id]; ?>"> 
<span id="msg"></span>
</li></ul>
<?php //}?>
        </fieldset>
      </div>
      <ul><li class="button"><input name="btnStudentedit" id="btnStudentedit" value="update" type="submit" class="submitBtn fl" />
