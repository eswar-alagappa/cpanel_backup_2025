...
$('.course').live('change',function(){
			if($(this).find(":selected").text()=="Fail"){				
				$(this).parent().next('td').find('select.ddlgrade').val('46');
				$(this).parent().next('td').find('select.ddlgrade').attr("disabled", "true");
			}else{
				$(this).parent().next('td').find('select.ddlgrade').removeAttr("disabled");
				}
			});
