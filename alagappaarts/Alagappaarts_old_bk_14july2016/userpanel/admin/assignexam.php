...
;
  ...
  if($studentexamassignedCourses) {
   ...
   <label>{$studentexamassignedCourse[code ]}</label>
	  ...
	  echo " <td height='38' class='schfromdate'><input name='existingvalue[{$studentexamassignedCourse[id]}][txtschdate1]' type='text' value='{$studentexamassignedCourse[exam_date_starttime]}' class='required datepicker examfromDate required date' title='Enter from date'/> </td>
    <td height='38' class='schtodate'><input name='existingvalue[{$studentexamassignedCourse[id]}][txtschdate2]' type='text' value='{$studentexamassignedCourse[exam_date_endtime ]}'  class='required datepicker examtoDate required date' title='Enter to date'/> </td>";
	  echo " </tr> ";   $i++ ;} 
     ...
     if($studentCourses ) { echo "<tr><td class='courselist1'>"; echo '<select name="assignExam[0][ddlcourse]"  class="required course" title="Select Course">  <option value="">Select</option>';  
	 foreach ($studentCourses as $studentCourse)
 				{ echo "<option value='{$studentCourse[id]}'>{$studentCourse[code]}</option>"; } 
	echo "</select>";
      
   echo "</td>
    <td height='38' class='schfromdate'><input name='assignExam[0][txtschdate1]' type='text' value=''  class='required datepicker examfromDate date' title='Enter from date'/> </td>
    <td height='38' class='schtodate'><input name='assignExam[0][txtschdate2]' type='text' value=''  class='required datepicker examtoDate date'  title='Enter to date'/> </td>";
	 } 
