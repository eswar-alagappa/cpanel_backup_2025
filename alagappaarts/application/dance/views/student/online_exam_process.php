...
<div class="wite">
	...
	<div class="topNav">
      <ul>
		<li><a href="javascript:;">Online Exams</a></li>
        <li class="last"> &nbsp; <?php echo $result->quiz_name ?></li>
...
?>
<div class="category_box1" id="cate-<?php echo $key;?>" <?php if($key=="0"){ echo "style='background:#2f72b7;color:#ffffff;'"; }?> onClick="changecategory('<?php echo $key;?>');"><?php //echo $result->quiz_name;//$cvalue; ?></div>
...
<form action="<?php echo site_url('dance/student/online_submit_quiz/');?>" method="post" id="testform" onsubmit="showquestion('0');">
<?php 
$category_number=0;
$TotalquestionCnt = count($assigned_question[0]);

$multiple = $fillup = $matchthefollowing = $subjective = array();
...
?>
	...
	?>
		...
		$category_number+=1;
		?>
		<a href="javascript:;"   class="btn btn-success"  onClick="showquestion('<?php echo $key+1;?>','<?php echo $question['q_type'];?>');changecategory('<?php echo $category_number;?>');update_curr_ans('<?php echo $key;?>','<?php echo $question['q_type'];?>');" > Save & Next</a>
		<?php
		}else{
		?>
	...
	if($option['qid']==$question['qid']){
		...
		<tr><td >
			<div class="questionTitle">
				<div class="questionTitleLeft">Fill in the Blank -  Question <?php echo (($key+1) - count($multiple));//$key+1; ?> of <?php echo count($fillup); ?> </div>
			</div>
		</td></tr>
		<tr><td> <div class="partContent"><div><?php echo $question['question'];?></div>
		<div class="fill-text">	
		<?php $fill_replace="<input type='text'  autocomplete='off' name='answers".$key."' value='' >"; //.$question_user_ans[$question['qid']].
	...
	?>
		...
		<tr><td >
			<div class="questionTitle">
				<div class="questionTitleLeft">Subjective 5 Marks - Question <?php echo $key+1 - ( count($fillup) + count($multiple) + count($matchthefollowing)); ?> of <?php echo count($subjective); ?>  </div>
			</div>
		</td></tr>
		<tr><td> <div class="partContent-subjective"><div><?php echo $question['question'];?>
		...
		&nbsp; <?php if ($result->pract_test=="0"){ ?><a href="javascript:sbtform();"   class="btn btn-danger">Submit Test</a><?php }else{ ?> <!--<a href="javascript:close_practice();"   class="button-error pure-button" style="cursor:pointer;">Close</a>-->
<a href="javascript:sbtform();"   class="btn btn-danger">Submit Test</a>		<?php } ?>
		<?php }?>
		</td></tr>
	...
	?>
		...
		<tr><td > 
			<div class="questionTitle">
				<div class="questionTitleLeft">Match the following - Question <?php echo $key+1 - ( count($fillup) + count($multiple)); ?> of <?php echo count($matchthefollowing); ?>  </div>
			</div>
		</td></tr>
		<tr><td> <div class="partContent"><div><?php echo $question['question'];?></div></td></tr>
		<?php 
		$opcount=0;
		$q_op=array();
		$q_ans=array();
...
<tr><td><table class="match-question">
		...
		$matchtheFollowingAns = $q_ans;//array_slice($q_ans,0,10);
		...
		<option value="">Select your option</option>
		 ...
		 <?php 
		 shuffle($matchtheFollowingAns);//shuffle($q_ans);
...
jQuery(document).ready(function($) {
  ...
  if (window.history && window.history.pushState) {
    ...
    window.history.pushState('forward', null, './online_exam_process');
...
$(document).bind('keydown keyup', function(e) 
 {
	...
	{   
		 var clickedcrr = confirm("CTRL + R is disable, If you press ok your exam result will not be stored");
			if(clickedcrr)
			{
				window.location = "<?= base_url('dance/student/event_error/'.$result->course_id.'~ctrlr') ?>";
			}
			else
			{
				return false;
	 ...
	 { 
		...
		if(isSafari == true)
		{
		    alert("F5 is disable If you press ok your exam result will not be stored");
		}
		else
		{
		  var clickedf5 = confirm("F5 is disable If you press ok your exam result will not be stored");
			
			if(clickedf5)
			{
				window.location = "<?= base_url('dance/student/event_error/'.$result->course_id.'~f5') ?>";
			}
			else
			{
				return false;
	 ...
	 else if(e.which === 116 && e.ctrlKey) 
	 { 
		var clickedctrlf5 = confirm("F5 is disable If you press ok your exam result will not be stored");
...
// {
        ...
        // if (e.keyCode == 116 || (e.ctrlKey && keycode == 116)) 
		// {
            // var clickedf5 = confirm("F5 is disable If you press ok your exam result will not be stored");
...
window.location="<?php echo site_url();?>dance/student/close_practice/";
}
