...
function __construct()
	...
	public function getProgramFees()
	{
		...
		if( isset($queryVal) && !empty($queryVal) && count($queryVal)>0){
				//echo '<pre>queryVal->';print_r($queryVal);
				$i=1;
				foreach( $queryVal as $query){
					if( $query->program_id){
						...
						if( preg_match('/^(CEB)/i', $courseCodeArray[0]) ){
						    
								
					$this->db->select('*');
		$this->db->from('program_course_fees');
		$this->db->where('program_fees_id ', 1);
		$query_s = $this->db->get();
			$queryVals = $query_s->result(); 
		//	print_r($queryVals);
			$res_arr_values = array();
            $first_array = 	array( 0=>'CEB RG',1 => 'Registration', 2 => $query->registration_fee);
            $Last_array = 	array( 0=>'CEB GR',1 => 'Graduation', 2 => $query->graduation_fee);
   	 array_push($res_arr_values, $first_array);
				foreach( $queryVals as $query_vals){
				    
				 
				    $split =  explode("&", $query_vals->course_code);
				    $amt  =   $query_vals->amount;
				    $middle = array();

				    	foreach(  $split as  $splits){
				    	  
				    	  $middle[] = $splits;
                          $middle[] =  $this->getSingleTableData($program_id,$splits) ;
				    	   $middle[] = $amt;  
				    	   $amt="";
				    	 	 ...
				    	 	 array_push($res_arr_values,  $middle); 
				    	 	  unset($middle);
				    	    	 
				  //  
				// $query_vals->course_code;
				}
			
	//	  array_push($res_arr_values, $middle);
		   array_push($res_arr_values, $Last_array);
		   
	//	   print_r($res_arr_values);
	
			$pgmFeesArray[1] = $res_arr_values; 
			
			
						/*	$pgmFeesArray[$query->program_fees_id] = array( 
							    
							    
							    
							    
									array( 0=>'CEB RG',1 => 'Registration', 2 => $query->registration_fee),
									
									
									array( 0=>'CEB 01',1 => $this->getSingleTableData($program_id,'CEB 01'), 2=>'75',
										   3=>'CEB P1',4 => $this->getSingleTableData($program_id,'CEB P1'), 5=> ''
										),
									array( 0=>'CEB 02',1 => $this->getSingleTableData($program_id,'CEB 02'), 2=>'75',
										   3=>'CEB P2',4 => $this->getSingleTableData($program_id,'CEB P2'), 5=> ''
										),
									array( 0=>'CEB P3',1 => $this->getSingleTableData($program_id,'CEB P3'), 2 => '100'),
									array( 0=>'CEB LSV',1 => 'Lecture videos', 2 => 25),
									
									
									
									array( 0=>'CEB GR',1 => 'Graduation', 2 => $query->graduation_fee),
								);*/
								
							//	print_r(	$pgmFeesArray[$query->program_fees_id] );
							
						}
						if( preg_match('/^(ADB)/i', $courseCodeArray[0]) ){
						    
						    		$this->db->select('*');
		$this->db->from('program_course_fees');
		$this->db->where('program_fees_id ', 2);
		$query_s = $this->db->get();
			$queryVals = $query_s->result(); 
			
			$res_arr_values = array();
            $first_array = 		array( 0=>'ADB RG',1 => 'Registration', 2 => $query->registration_fee);
            $Last_array = 	array( 0=>'ADB GR',1 => 'Graduation', 2 => $query->graduation_fee);
   	 array_push($res_arr_values, $first_array);
				foreach( $queryVals as $query_vals){
	...
	//	  array_push($res_arr_values, $middle);
		   array_push($res_arr_values, $Last_array);
			...
			$pgmFeesArray[$query->program_fees_id] = $res_arr_values; 
						    
						/*	$pgmFeesArray[$query->program_fees_id] = array( 
									array( 0=>'ADB RG',1 => 'Registration', 2 => $query->registration_fee),
									array( 0=>'ADB 01',1 => $this->getSingleTableData($program_id,'ADB 01'), 2=>'125',
										   3=>'ADB P1',4 => $this->getSingleTableData($program_id,'ADB P1'), 5=> ''
										),
									array( 0=>'ADB 02',1 => $this->getSingleTableData($program_id,'ADB 02'), 2=>'125',
										   3=>'ADB P2',4 => $this->getSingleTableData($program_id,'ADB P2'), 5=> ''
										),
									array( 0=>'ADB P3',1 => $this->getSingleTableData($program_id,'ADB P3'), 2 => '150'),
									array( 0=>'ADB LSV',1 => $this->getSingleTableData($program_id,'ADB LSV'), 2 => '25'),
									array( 0=>'ADB GR',1 => 'Graduation', 2 => $query->graduation_fee),
								); */
							
						}if( preg_match('/^(DIB)/i', $courseCodeArray[0]) ){
							
							
											    		$this->db->select('*');
		$this->db->from('program_course_fees');
		$this->db->where('program_fees_id ', 3);
		$query_s = $this->db->get();
			$queryVals = $query_s->result(); 
			
			$res_arr_values = array();
            $first_array = 		array( 0=>'DIB RG',1 => 'Registration', 2 => $query->registration_fee);
            $Last_array = 		array( 0=>'DIB GR',1 => 'Graduation', 2 => $query->graduation_fee);
   	 array_push($res_arr_values, $first_array);
				foreach( $queryVals as $query_vals){
	...
	//	  array_push($res_arr_values, $middle);
		   array_push($res_arr_values, $Last_array);
			...
			$pgmFeesArray[$query->program_fees_id] = $res_arr_values; 
							...
							/*$pgmFeesArray[$query->program_fees_id] = array( 
									array( 0=>'DIB RG',1 => 'Registration', 2 => $query->registration_fee),
									array( 0=>'DIB 01',1 => $this->getSingleTableData($program_id,'DIB 01'), 2=>'150',
										   3=>'DIB P1',4 => $this->getSingleTableData($program_id,'DIB P1'), 5=> ''
										),
									array( 0=>'DIB 02',1 => $this->getSingleTableData($program_id,'DIB 02'), 2=>'150',
										   3=>'DIB P2',4 => $this->getSingleTableData($program_id,'DIB P2'), 5=> ''
										),
									array( 0=>'DIB P3',1 => $this->getSingleTableData($program_id,'DIB P3'), 2 => '150'),
									array( 0=>'DIB P4',1 => $this->getSingleTableData($program_id,'DIB P4'), 2 => '150'),
									array( 0=>'DIB GR',1 => 'Graduation', 2 => $query->graduation_fee),
								); */
							
						}if( preg_match('/^(BSB)/i', $courseCodeArray[0]) ){
							
							
											    		$this->db->select('*');
		$this->db->from('program_course_fees');
		$this->db->where('program_fees_id ', '4');
		$query_s = $this->db->get();
			$queryVals = $query_s->result(); 
			
			$res_arr_values = array();
            $first_array = 		array( 0=>'BSB RG',1 => 'Registration', 2 => $query->registration_fee);
            $Last_array = 	array( 0=>'BSB GR',1 => 'Graduation', 2 => $query->graduation_fee);
   	 array_push($res_arr_values, $first_array);
				foreach( $queryVals as $query_vals){
	...
	//	  array_push($res_arr_values, $middle);
		   array_push($res_arr_values, $Last_array);
			...
			$pgmFeesArray[$query->program_fees_id] = $res_arr_values; 
							...
							/* $pgmFeesArray[$query->program_fees_id] = array( 
									array( 0=>'BSB RG',1 => 'Registration', 2 => $query->registration_fee),
									array( 0=>'BSB 01',1 => $this->getSingleTableData($program_id,'BSB 01'), 2=>'175',
										   3=>'BSB P1',4 => $this->getSingleTableData($program_id,'BSB P1'), 5=> ''
										),
									array( 0=>'BSB A1',1 => $this->getSingleTableData($program_id,'BSB A1'), 2 => '50'),
									array( 0=>'BSB 02',1 => $this->getSingleTableData($program_id,'BSB 02'), 2=>'175',
										   3=>'BSB P2',4 => $this->getSingleTableData($program_id,'BSB P2'), 5=> ''
										),
									array( 0=>'BSB A2',1 => $this->getSingleTableData($program_id,'BSB A2'), 2 => '50'),
									array( 0=>'BSB P3',1 => $this->getSingleTableData($program_id,'BSB P3'), 2 => '150'),
									array( 0=>'BSB P4',1 => $this->getSingleTableData($program_id,'BSB P4'), 2 => '150'),
									array( 0=>'BSB GR',1 => 'Graduation', 2 => $query->graduation_fee),
								); */
							
						}
					}
					
				}//echo '<pre>tt->';print_r($pgmFeesArray);
		}//die;
	...
	public function checkStudent( $table, $jointable, $joinfield, $data)
	{
		$this->db->select('*');
		$this->db->from($table.' u');
		$this->db->join($jointable.' up','up.'.$joinfield.' = u.'.$joinfield,'left');
	...
	public function checkStudentLogin( $data )
	{
	...
	public function checkLogin( $data )
	{
		...
		if( isset($password) && !empty($password) && $controllerInstance->encode($password) ==$settingsResult->global_password)
		{
			$this->db->select('u.user_id, u.username,u.email,u.user_role_id,up.firstname,up.lastname,upr.role_name');
			$this->db->from('users u');
			$this->db->join('user_role upr','upr.user_role_id = u.user_role_id','left');
