...
function __construct()
   ...
   public function getListactiveState($table)
	...
	public function getStudentData( $table, $table2, $field, $user_id)
	{
		...
		$this->db->select('u.*,up.*,up.dob as user_dob,up.city as ucity, up.state as ustate, up.country as ucountry, up.zip as uzip, u.email as uemail, up.address as uaddress');
	...
	public function checkstudentPassword($pass, $user_id)
	{
		$this->db->select('*');
...
/*	 function assigned_questions_manually($quid,$course_id){
 ...
 $institute_id = 1;//1;
 	...
 	if($qids !=""){ 
 	$qrr="SELECT qbank.*,question_category.*,difficult_level.level_name FROM qbank JOIN question_category ON qbank.cid = question_category.cid JOIN difficult_level on qbank.did=difficult_level.did where qbank.course_id=$course_id and qid in ($qids) ORDER BY FIELD(qbank.qid, $qids ) ";
