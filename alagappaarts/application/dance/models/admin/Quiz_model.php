...
function __construct()
 ...
 function add_quiz(){
	...
	$insert_data = array(
			...
			'view_answer' => 1,//$this->input->post('view_answer'),
...
$kk=$k+1;
 ...
 function quiz_verify($id){
 	$institute_id = 1;
	if($this->input->cookie('rid', TRUE)){ //echo 'a';die;
...
// fetch quiz detail
	...
	$qrr=$query->row_array();
  		if($this->config->item('protocol')=="smtp"){
			...
			delete_cookie("rid");
			return "Quiz submitted successfully. <a href='".site_url('dance/admin/result/view/'.$rid.'/'.$id)."'>Click here</a> to view result";
