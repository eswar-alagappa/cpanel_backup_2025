...
function __construct()
	...
	public function getNewList($table,$program_id,$center_id,$status)
	{
			$this->db->select('*');
			$this->db->from($table.' p');
			$this->db->where('p.status', '1');
	...
	public function getNewSelected($selected_field, $id)
	{
		if( !empty($selected_field) && !empty($id) )
		{
			$this->db->select('*');
			$this->db->from('new_students p');
			$this->db->where('p.'.$selected_field, $id);
			$query = $this->db->get();
	...
	public function isNewDelete($table, $changeField, $category_id)
	{
		$this->db->select( array($changeField));
		$this->db->from($table);
		$this->db->where($changeField, $category_id);
