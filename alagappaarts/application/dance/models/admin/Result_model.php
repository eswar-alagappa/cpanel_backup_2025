...
Class Result_model extends CI_Model
{
 ...
 // to search
  ...
  $query = $this->db->query("SELECT * FROM  `quiz_result` JOIN users ON quiz_result.uid = users.user_id JOIN user_profiles as up ON up.user_id=users.user_id JOIN quiz ON quiz_result.quid = quiz.course_id where $search_type like '%$search%' $condi and quiz_result.institute_id = '$institute_id' and quiz_result.quid!=0 and qids!='' order by quiz_result.rid desc limit $limit, $nor");

 
 }else{
	 
   $query = $this->db->query("SELECT * FROM  `quiz_result` JOIN users ON quiz_result.uid = users.user_id JOIN user_profiles as up ON up.user_id=users.user_id JOIN quiz ON quiz_result.quid = quiz.course_id $condi  and quiz_result.institute_id = '$institute_id' and quiz_result.quid!=0 and qids!='' order by quiz_result.rid desc limit $limit, $nor");
...
$res=array();
  ...
  function remove_result($rid){
	  ...
	  $select = $this -> db
           ...
           -> limit(1)
           -> get('quiz_result');
	  $deleted_data = $select->result_array()[0];
	  $this->db->insert('quiz_result_deleted',$deleted_data);
 ...
 $institute_id =1;
   if($user_id!=""){
   $condi="and users.user_id='".$user_id."'";
 	}else{
 	$condi="";
 	}
	
  //$query = $this->db->query("SELECT * FROM  `quiz_result` JOIN users ON quiz_result.uid = users.user_id JOIN user_profiles as up ON up.user_id=users.user_id JOIN quiz ON quiz_result.quid = quiz.course_id where quiz_result.rid='$id' and quiz_result.institute_id = '$institute_id' $condi");
  $query = $this->db->query("SELECT ae.*, quiz_result.*,up.*,quiz.*,users.* FROM  `quiz_result` JOIN users ON quiz_result.uid = users.user_id JOIN assign_exam as ae ON ae.id= quiz_result.assign_exam_id JOIN user_profiles as up ON up.user_id=users.user_id JOIN quiz ON quiz_result.quid = quiz.course_id where quiz_result.rid='$id' and quiz_result.institute_id = '$institute_id' $condi");
...
$query = $this->db->query("SELECT * FROM  q_options WHERE oid='$oi' ");
  ...
  $institute_id =1;
	...
	//echo '<pre>questions_opt->';print_r($questions_opt);die;
 ...
 $this->db->join('users','users.user_id = quiz_result.uid');
