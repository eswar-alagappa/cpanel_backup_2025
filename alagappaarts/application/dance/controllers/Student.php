...
class Student extends CI_Controller {
	 ...
	 *	- or -
	 * 		http://example.com/index.php/welcome/indexonline_exam
	 *	- or -
	...
	public function checkStudentOldPass( $str )
	{
		$user_id = $this->session->userdata('site_user_id');
		$oldPass = $this->encode($str);
		$check = $this->Student_model->checkstudentPassword($oldPass,$user_id);
		if( !empty($check))
		{
			return true;
		}else{
			$this->form_validation->set_message('checkStudentOldPass', 'The %s field has wrong. please enter correct password ');
			return false;
		}
	}
	
	
	public function student_syllabus_view($id)
	{
	    
	    if($this->session->userdata('site_logged_in') == false){
			redirect(base_url().'dance', 'refresh');
		}
	    $syllabusList = $this->Student_model->getConditionalRecord('syllabus_program','syllabus_program_id',$id);
		...
		$this->load->view('student/student_syllabus_view',$data);
		$this->load->view('student/footer');
	...
	public function course_details()
	{
		...
		$this->load->view('student/studentheader');	
		$this->load->view('student/student_course_details');
		$this->load->view('student/footer');
	...
	public function student_syllabus()
	{
		...
		$getCurrentProram = $this->Student_model->getConnectedDataList('students','student_syllabus_program','user_id','user_id',$user_id);
		rsort($getCurrentProram);
		
		$studentCurrentProgarmId = $getCurrentProram[0]->program_id;
	    
	    $syllabusList = $this->Student_model->getConditionalRecord('syllabus_program','program_id',$studentCurrentProgarmId);
		...
		$this->load->view('student/studentheader');	
		$this->load->view('student/student_syllabus',$data);
		$this->load->view('student/footer');
		
	}
	
	public function newstudent_syllabus()
	{
	    if($this->session->userdata('site_logged_in') == false){
			redirect(base_url().'dance', 'refresh');
		}
		...
		$this->load->view('student/studentheader');	
		$this->load->view('student/newstudent_syllabus',$data);
		$this->load->view('student/footer');
	...
	public function student_change_password()
	{
		...
		if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['btnChangepassword']) && $_POST['btnChangepassword']=='Submit') ) {
			...
			$this->form_validation->set_rules('txtOldpassword', 'Old Password', 'trim|required|min_length[8]|callback_checkStudentOldPass');
			$this->form_validation->set_rules('txtNewPassword', 'New Password', 'trim|required|min_length[8]|matches[txtRenewpassword]');	
			$this->form_validation->set_rules('txtRenewpassword', 'Renew Password', 'trim|required|min_length[8]');
			...
			if($this->form_validation->run() == FALSE )
			{
				...
				$set_user_data = array(
				 ...
				 $check = $this->Student_model->update('students','user_id',$set_user_data,$user_id);
				 if( isset($check) && !empty($check)){
					 
					 $userData = $this->Student_model->getStudentData( 'students','student_profiles','user_id',$user_id);
					 $content = 'Password has changed Successfully';
						$subject = 'Password change to APAA!';
						$user_data = array(
							'firstname'	=> $userData->firstname,
					...
					$this->session->set_flashdata('SucMessage', 'Invalid Details');
					redirect(base_url().'dance/student/student_change_password'); 
				}
	...
	public function edit_studentprofile()
	{
				...
				if($this->form_validation->run() == FALSE )
				{
					...
					$this->ErrorMessages		 		= validation_errors();
						...
						$set_user_data = array(	
						 ...
						 $check = $this->Student_model->update('students','user_id',$set_user_data,$user_id);
						if( isset($check) && !empty($check)){
							$another_check = $this->Student_model->update('student_profiles','user_id',$set_userprofile_data,$user_id);
							$this->session->set_flashdata('SucMessage', 'Successfully created');
							redirect(base_url().'dance/student/studentprofile'); 
						}else{
							$this->session->set_flashdata('SucMessage', 'Invalid Details');
							redirect(base_url().'dance/student/edit_studentprofile'); 
						}
		...
		$this->load->view('student/studentheader');	
		$this->load->view('student/update_studentprofile',$data);
		$this->load->view('student/footer');
		...
		public function newstudentprofile()
	{
		...
		//echo '<pre>$data->';print_r($data);die;
		$this->load->view('student/studentheader');	
		$this->load->view('student/newstudentprofile',$data);
		$this->load->view('student/footer');
	}
	
		public function view_studentprofile()
	{
	    if($this->session->userdata('site_logged_in') == false){
			redirect(base_url().'dance', 'refresh');
		}
		...
		//echo '<pre>$data->';print_r($data);die;
		$this->load->view('student/studentheader');	
		$this->load->view('student/view_studentprofile',$data);
		$this->load->view('student/footer');
	}
	...
	{	
		...
		// $alert = '<script>alert("your exam result will not be stored");</script>';
		// echo '<pre>';
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Register') ) {
				...
				if($this->form_validation->run() == FALSE )
				{
					...
					if( isset($check) && !empty($check)){
						...
						$content = 'Greetings from APAA! Your request for registering as APAA Dance Student has been successfully completed. The APAA customer support team will get back to you shortly.';
    ...
    public function new_registration()
	{
        ...
        $this->form_validation->set_rules('txtDOB', 'DOB', 'trim|required');
        $this->form_validation->set_rules('txtGender', 'Gender', 'trim|required');
        $this->form_validation->set_rules('txtMobileNo', 'Mobile No', 'trim|required');
        $this->form_validation->set_rules('txtEmail', 'Email', 'trim|required|valid_email');
        
        $this->form_validation->set_rules('txtFatherName', 'Father Name', 'trim|required');
        $this->form_validation->set_rules('txtMotherName', 'Mother Name', 'trim|required');
        ...
        $this->form_validation->set_rules('txtZip', 'Zip', 'trim|required');
        $this->form_validation->set_rules('txtLevel', 'Level', 'trim|required');
        $this->form_validation->set_rules('txtNameofIns', 'Name of Institution', 'trim|required');
       /* $this->form_validation->set_rules('txtAcaCity', 'City', 'trim|required');
        $this->form_validation->set_rules('txtAcaState', 'State', 'trim|required');
        $this->form_validation->set_rules('txtAcaCountry', 'Country', 'trim|required');*/
        $this->form_validation->set_rules('txtExpInBhar', 'Experiance in Bharathanatyam', 'trim|required');
        $this->form_validation->set_rules('txtNameofGuru', 'Name of your Guru', 'trim|required');
        $this->form_validation->set_rules('txtBharMobileNo', 'Mobile', 'trim|required');
        $this->form_validation->set_rules('txtBharAddress', 'Address', 'trim|required');
        $this->form_validation->set_rules('txtSpecqualification', 'Special accomplishments', 'trim|required');
        $this->form_validation->set_rules('txtNameofDanceIns', 'Name of the Dance Institution', 'trim|required');
        /*$this->form_validation->set_rules('txtBharCountry', 'Country', 'trim|required');
        $this->form_validation->set_rules('txtBharCity', 'City', 'trim|required');
        $this->form_validation->set_rules('txtBharState', 'State', 'trim|required');
        $this->form_validation->set_rules('txtBharZip', 'Zip', 'trim|required');*/
        $this->form_validation->set_rules('txtRegular', 'Program Enrolled', 'trim|required');
				...
				$BirthfileName = $Birthflag = '';
				if( isset($_FILES['fleBirth']['name']) &&  !empty($_FILES['fleBirth']['name']) && trim($_POST['uploadBirthImage'])=='' )
				{
					$Birthflag = 1;
				}else if( isset($_FILES['fleBirth']['name']) &&  !empty($_FILES['fleBirth']['name']) && trim($_POST['uploadBirthImage'])!='' )
				{
					unlink('../assets/profile/'.$_POST['uploadBirthImage']);
					$Birthflag = 1;
				}else if( isset($_FILES['fleBirth']['name']) &&  empty($_FILES['fleBirth']['name']) && trim($_POST['uploadBirthImage'])!='' )
				{
					$Birthflag = 2;
        ...
        if( isset($Birthflag) && $Birthflag==1)
		{
        $fileExtension = explode(".", $_FILES['fleBirth']['name']);
        $fileExt = array_pop( $fileExtension );
        $fileName = strtolower(  md5(time()) .".".$fileExt);
        $config['file_name'] = $fileName;
        $config['upload_path'] = '../assets/profile';
        $config['allowed_types'] = 'jpg|png|jpeg';
          ...
          if(!$this->upload->do_upload('fleBirth'))
          {
            $UploadError 	= array('error' => "Please select a valid 100kb size file");		
          }else{
            ...
            { 
              $thumbImage = 'thumb_'.$fileName;
              if( !empty($_POST['uploadBirthImage'])){
                $_POST['uploadBirthImage'] = $_POST['uploadBirthImage'];
              }else{
                $_POST['uploadBirthImage'] = $post_set['uploadBirthImage'] = 'thumb_'.$fileName;
                unlink('../assets/profile/'.$fileName);
              }
            }
          }
        ...
        if( isset($Birthflag) && $Birthflag==1)
		{
		    $fileExtension = explode(".", $_FILES['fleBirth']['name']);
            $fileExt = array_pop( $fileExtension );
            $fileName_birth = strtolower(  md5(time()) ."_birth.".$fileExt);
            $config_birth['file_name'] = $fileName_birth;
            $config_birth = '../assets/profile';
            $config_birth['allowed_types'] = 'jpg|png|jpeg';
            ...
            //$config['max_width']  = '157';
              //strtolower(str_replace(' ', '-',$fileExtension[0]))
              //echo '<pre>';print_r($config);
              $this->load->library('upload', $config_birth);
              ...
              if(!$this->upload->do_upload('fleBirth'))
              {
                ...
                $upload_data = $this->upload->data();
                rename("../assets/profile/".$upload_data['file_name']."", "../assets/profile/".$fileName_birth."");
              }
		}
				...
				if($this->form_validation->run() == FALSE )
				{
					...
					//echo '<pre>';print_r($_POST);die;
					$new_students_profiles = array(						
							'firstname' 		 	 			=> addslashes(trim( $this->input->post('txtFirstName') )),
							'lastname' 		 	 				=> addslashes(trim( $this->input->post('txtLastName') )),
							'age' 		 	 					  => addslashes(trim( $this->input->post('txtAge') )),
              'dob' 		 	 					  => addslashes(trim( $this->input->post('txtDOB') )),
              'gender' 		 	 					=> addslashes(trim( $this->input->post('txtGender') )),
              'photo' 		 	 			    => $fileName,
              'birth_certificate' 		 	 			    => $fileName_birth,
							'mobileno' 		 	 				=> addslashes(trim( $this->input->post('txtMobileNo') )),
							'altermobileno' 		 	 	=> addslashes(trim( $this->input->post('txtAlterMobileNo') )),
              'email' 		 	 					=> addslashes(trim( $this->input->post('txtEmail') )),
              
              'father_name' 		 	 					=> addslashes(trim( $this->input->post('txtFatherName') )),
              'father_occ' 		 	 					=> addslashes(trim( $this->input->post('txtFatherOcc') )),
              'mother_name' 		 	 					=> addslashes(trim( $this->input->post('txtMotherName') )),
              'mother_occ' 		 	 					=> addslashes(trim( $this->input->post('txtMotherOcc') )),
              
              'address' 		 	 				=> addslashes(trim( $this->input->post('txtAddress') )),
              'city' 		 	 			      => addslashes(trim( $this->input->post('txtCity') )),
							'state' 		 	 				  => addslashes(trim( $this->input->post('txtState') )),
							'country' 		 	 				=> addslashes(trim( $this->input->post('txtCountry') )),
              'zip' 		 	 					  => addslashes(trim( $this->input->post('txtZip') )),
              'level' 		 	 					=> addslashes(trim( $this->input->post('txtLevel') )),
              'nameofins' 		 	 			=> addslashes(trim( $this->input->post('txtNameofIns') )),
			/*				'acacity' 		 	 				=> addslashes(trim( $this->input->post('txtAcaCity') )),
							'acastate' 		 	 				=> addslashes(trim( $this->input->post('txtAcaState') )),
              'acacountry' 		 	 			=> addslashes(trim( $this->input->post('txtAcaCountry') )),*/
              'expinbhar' 		 	 			=> addslashes(trim( $this->input->post('txtExpInBhar') )),
              'specquali' 		 	 			=> addslashes(trim( $this->input->post('txtSpecqualification') )),
							'nameofguru' 		 	 			=> addslashes(trim( $this->input->post('txtNameofGuru') )),
              'nameofdance' 		 	 		=> addslashes(trim( $this->input->post('txtNameofDanceIns') )),
							'bharmobileno' 		 	 		=> addslashes(trim( $this->input->post('txtBharMobileNo') )),
              'bharaltermobno' 		 	 	=> addslashes(trim( $this->input->post('txtBharAlterMobileNo') )),
              'bharaddress' 		 	 		=> addslashes(trim( $this->input->post('txtBharAddress') )),
              /*'bharcity' 		 	 			  => addslashes(trim( $this->input->post('txtBharCity') )),
							'bharstate' 		 	 			=> addslashes(trim( $this->input->post('txtBharState') )),
							'bharcountry' 		 	 		=> addslashes(trim( $this->input->post('txtBharCountry') )),
              'bharzip' 		 	 				=> addslashes(trim( $this->input->post('txtBharZip') )),*/
              'program_enroll' 		 	 	=> addslashes(trim( $this->input->post('txtRegular') )),
							'status'								=> 1,
							'created_by'						=> 'User',
							'updated_by'						=> 'User',
							'created_at'						=> date('Y-m-d H:i:s'),
					...
					$check = $this->db->insert('new_students',$new_students_profiles);
					if( isset($check) && !empty($check)){
						$content = 'Greetings from APAA! Your request for registering as APAA Dance Student has been successfully completed. The APAA customer support team will get back to you shortly.';
						$subject = 'Greetings from APAA!';
						$user_data = array(
						...
						$this->session->set_flashdata('SucMessage', 'Thanks for your Registration. Please wait for Admin approval');
						redirect(base_url().'dance/student/new_registration'); 
					}else{
						$this->session->set_flashdata('ErrMessage', 'Invalid Details');
						redirect(base_url().'dance/student/new_registration'); 
					}
		...
		$this->setHomeHeader();
		$this->load->view('student/new_registration',$data);
		$this->load->view('student/footer');
	}
    /* Code By Vignesh Kumar */
	...
	public function online_exam()
	{
		...
		if( isset($user_id) && !empty($user_id))
		{
			...
			$getExamSchedule = $this->Student_model->getExamSchedule($user_id)[0];
	...
	public function exam_process( $course = '')
	{
		...
		{ 
			if( isset($course) && !empty($course) && $course != 'online_exam_process' )
			{
				...
				{
					$getAssignedExam = $this->Student_model->getExamWithID( $user_id,$course_id );	
					$qid = $course_id;
					$assign_exam_id = $getAssignedExam[0]->id;
				}
				else
				{
			...
			//echo '<pre>session->';print_r($_SESSION);die;
			...
			log_message('error','quiz_qid ->'.print_r( json_encode($quiz_qid),TRUE));
			
			/*********** code hidden on 27-03-2018 ******/
			
			// // if( isset($checkBendingExam) && !empty($checkBendingExam) && $checkBendingExam[0]->quid != $quiz_qid && empty($checkBendingExam[0]->score_ind))
			// // {				
				// // //$this->session->set_flashdata('ErrMessage', 'Already Your exam has Pending . course code '.$checkBendingExam[0]->course_code.' Please Contact APAA Support team.');
				// // redirect('dance/student/online_exam/', 'refresh');				
			// // }
			// // else
			// // {  
				...
				{
					$id = $_SESSION['qid'] = $qid;
					$_SESSION['assign_exam_id'] = $assign_exam_id;
				}
		...
		if( isset($_SESSION['qid']) && !empty($_SESSION['qid']))
		{
			...
			{  
				...
				{				
						...
						if( isset($data['user_answer']) && !empty($data['user_answer']))
						{
							foreach($data['user_answer'] as $val_ans)
							{
								$question_user_ans[$val_ans['q_id']]=$val_ans['ess'];				
							}
						}
						//echo '<pre>';print_r($question_user_ans);die;
						$data['question_user_ans']=$question_user_ans;
						...
						if( isset($data['time_info']) && !empty($data['time_info']))
						{
							...
							{
								$this->Student_model->Online_Exam_Processing_status( 'exam_startdate', date('Y-m-d H:i:s'), 'Processing', $getAssignedExam[0]->id,$user_id);
							}
			...
			{	 
				...
				$data['resultstatus'] = $this->Student_model->quiz_verify($quiz_id,$assign_exam_id);
				$data['quiz_id']=$quiz_id;
				$data['result'] = $this->Student_model->quiz_detail($quiz_id);
				$data['title']=$data['result']->quiz_name;
				if( isset($data['resultstatus']) && !empty($data['resultstatus']) && $data['resultstatus'] == "1")
	...
	public function online_exam_process()
	{
		...
		if(!empty($_SESSION['assign_exam_id']) && !empty($_SESSION['session_rid']) && !empty($_SESSION['qid']))
		{	
		
			if(!empty($_SESSION['qid']))
			{
			
				$result = $this->Student_model->dbQuery('select * from quiz_result where uid = "'.$this->session->userdata('site_user_id').'" and quid = "'.$course_id.'" ');

				 $program_id = $this->Student_model->dbQuery('select * from courses where course_id = "'.$_SESSION['qid'].'" ');
				 
				 if($result[0]['rid'] =="")
				 {
					$result_id =  $_SESSION['session_rid'];
				 }
				 else
				 {
					 $result_id = $result[0]['rid'];
				 }
				 
				 $data = array(
						'uid' => $this->session->userdata('site_user_id'),
						'rid' => $result_id,
						'course_id' => $_SESSION['qid'],
						'program_id' => $program_id [0]['program_id'],
						'error_report' => "Browser Reload Press or copy Paste Url",
						'created_at' => date('Y-m-d H:i:s'),
				 );
			 
				$this->Student_model->insert_values('error_event',$data);	
				
					$_SESSION['msg'] = '<center><div class="alert alert-success">You exam Process failed Due to Browser Reload pressed or copy page exam url. Please Contact APAA support Team.</div></center>';							
			
				$_SESSION['qid'] = '';
				$_SESSION['session_rid'] = '';
				$_SESSION['assign_exam_id'] = '';		
				$this->session->unset_userdata('qid');
				$this->session->unset_userdata('assign_exam_id');
				$this->session->unset_userdata('session_rid');
				$this->session->unset_userdata('__ci_last_regenerate'); 
							
				redirect(base_url().'dance/student', 'refresh');
	...
	{		
		...
		$assign_exam_id = ((isset( $_SESSION['assign_exam_id'] ) && !empty($_SESSION['assign_exam_id'])) ? $_SESSION['assign_exam_id'] : '');
		...
		{ //echo '<pre>session->';print_r($_SESSION);die;			
			...
			if( isset($assign_exam_id) && !empty($assign_exam_id))
			{		
				$getAssignedExam = $this->Student_model->getSelected( 'assign_exam','id',$assign_exam_id );
				if((isset($getAssignedExam) && !empty($getAssignedExam)))
				{
					$findStatus = $this->Student_model->Online_Exam_Processing_status( 'exam_completiondate', date('Y-m-d H:i:s'), 'Processing', $getAssignedExam->id,$user_id );
				...
				$tws_userid=$this->session->userdata('site_user_id');
				$this->db->select('*');
    			$this->db->from('users');
    			$this->db->where('user_id',$tws_userid);
    			$get_user_details = $this->db->get();
    			if($get_user_details->num_rows()>0){
    			foreach($get_user_details->result() as $get_user_details_row){
    			$user_email_id = $get_user_details_row->email;
    			}
    			}else{
    			$user_email_id = '';    
    			}
				...
				$subject = 'You have successfully completed your examination';
				$user_data = array(
					'firstname'	=> addslashes(trim( $firstname )),
					'lastname'	=> addslashes(trim( $lastname )),
					'email'		=> addslashes(trim( $user_email_id ))
				);
				
				$content = "
				<p>Dear ".$firstname." ".$lastname.",<br />
                Greeting from APP!!!<br />
                You have successfully completed your examination.Result will be published in 3 to 4 week time.certificate & Consolidated mark sheet will be issued in 3 Months time as per Universitynorm..<br />
                <br />
	...
	public function event_error($field)	
	{	
		$value = explode('~',$field);
	    $course_id = $value[0];
		...
		if($value[1] == "f5")
		{
			$error_msg = "F5 Button Pressed";
		}
		if($value[1] == "ctrlr")
		{
			$error_msg = "CTRL + R Key Pressed";
		}
		...
		{			
		 ...
		 $program_id = $this->Student_model->dbQuery('select * from courses where course_id = "'.$course_id.'" ');
		 ...
		 $data = array(
			...
			$_SESSION['msg'] = '<center><div class="alert alert-success">Your Exam Failed Reason ' .$error_msg. ' While Writing Exam. Please Contact APAA support Team.</div></center>';
