...
class Centers extends CI_Controller {
		...
		if(isset($arg) && !empty($arg))
		{
				...
				if( $user_data->status ==1)
				{
					...
					$this->session->set_flashdata('SucMessage', 'Successfully Send Mail');
						redirect(base_url().'dance/admin/centers/index'); 
				}
	...
	{	
		...
		$listData = $this->Student_model->fetchStudent(3);
...
personalized details and your students details such as Profile, online
exam schedules, exam results, fees payment, etc.</p>
<p><a target="_blank" href="'.$login.'">'.$login.'</a></p>';
			$subject = 'APAA-New login info';
			$user_data = array(
				'firstname'	=> addslashes(trim( $listData[$i]->directorName )),
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && !empty($id) && (!empty($_POST['update']) && $_POST['update']=='Update') ) {
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if($check){
						...
						$pass = ((isset($pass->password) && !empty($pass->password)) ? $this->decode($pass->password) : '');
						$content = '
Greetings from APAA! Your request for registering as APAA Dance Center has been successfully completed. The APAA customer support team has verified your Requested Profile. See below Your Username and Password <br>Username : '.$uname.'<br>Password : '.$pass.'';
