...
class Login extends CI_Controller {
	...
	{	
        ...
        {
            redirect(base_url().'dance/admin/login');die;
        }
		else if ( isset($admin_logged_in) && $admin_logged_in == 1)
        { 
            redirect(base_url().'dance/admin/master/dashboard');die;
        }
		$post_set		 		                	= $_POST;
			...
			if($this->form_validation->run() 		== FALSE)
			{
				...
				if($loginChk->su == "") {
					...
					if( !empty($loginChk->user_role_id) && $loginChk->user_role_id ==1){ 
						redirect(base_url().'dance/admin/user');
					}else{ 
					...
					redirect(base_url().'dance/admin/login');
				}
			}
