...
class Students extends CI_Controller {
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) 
			{
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($check) && !empty($check)){
    					...
    					$this->db->where('program_id',$pro_id);
    					$this->db->where('status','1');
    					$get_mail_content = $this->db->get();
    					if($get_mail_content->num_rows()>0){
    					foreach($get_mail_content->result() as $get_mail_content_row){
    					$mail_content = $get_mail_content_row->mail_content;
    					}
    					}else{
    					$mail_content = '';    
    					}
    					$mail_content = str_replace( 'usernamexxxxx', $uname, $mail_content );
    					$mail_content = str_replace( 'passwordxxxxx', $pass, $mail_content );
						$content = $mail_content;
						/*$content = '
						<p>Greetings from APAA</p>
						<p> We thank you for enrolling in the APAA Certificate Programme in Bharathanatyam. Find below, your login ID and Password:</p>
						<p>Find below, your login ID and Password to access www.alagappaarts.com.</p>
						<p><b>Username :</b> '.$uname.'</p><p><b>Password :</b> '.$pass.' </p>
						<p>Here you can find the syllabus, access sample tests, results and other relevant information about your course.</p>
						<p><a target="_blank" href="'.$login.'">'.$login.'</a></p>
						<p>Whereas your Guru will teach you the dance steps, APAA will conduct exams at the end of the course - 9 to 12 months from the start of the course. The course entails two semesters, with examinations at the end of each semester.</p>
						<p>There are three elements to examination.</p>
						<p>The theory exam will be online, with questions of the following types:</p>
					    <p>
                        <ul>
                        <li> Multiple Choice Questions</li>
                        <li> Short and Long Answers</li>
                        </ul>
                        </p>
						<p>Secondly, you will be required to provide a dance score featuring the following:</p>
						<p>
						<ul>
						<li> Adavus,</li>
                        <li> Korvai</li>
                        <li> Hastas</li>
                        <li> Movements of head, Neck and eyes,  </li>
                        <li> Alarippu</li>
                        <li> Jatheeswaram </li>
                        <li> Shabdam</li>
                        </ul>
                        </p>
						<p>Additionally, you will be required to undertake project work on one of the following topics:</p>
						<p><ul>
						<li>  How can the hastas be used in day to day life?</li>
                        <li>	What are the different adavus used in Korvais?</li>
                        <li>	Write a short note on the Charis?</li>
                        </ul>
						</p>
						<p>These individual projects can be between 500-1000 words.</p>
						<p>You can submit the online theory, examination, dance score, and written project to the following link  https://bit.ly/apaaupload</p>
						<p><b style="color:#fb0707">Note:</b></p>
						<p>The file types allowed- doc, pdf, Videos-mp4</p>
						<p>Maximum file size allowed is 1 GB</p>
						<p>Results will be announced online, and consolidated marks will be sent as manual certificates around 6 to 8 weeks after completing all requirements.</p>
						';*/
	...
	{	
		...
		/*if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['search']) && $_POST['search']=='Go') ) {
			...
			$center_id	= $_POST['center'];
			$status		= $_POST['status'];
			$listData 		= $this->Student_model->getNewList('new_students',$program_id,$center_id,$status);
			
		}else{
			$listData 		= $this->Student_model->getNewList('new_students','','','');
		}*/
		$listData 		= $this->Student_model->getNewList('new_students','','','');
		//echo '<pre>';print_r($listData);die;
		...
		$this->load->view('admin/header',$data);
		$this->load->view('admin/sidebar');
		$this->load->view('admin/students/new_list',$data);
		$this->load->view('admin/footer');
	}
	public function new_update($id=null)
	{
		if($this->session->userdata('admin_logged_in') == False)
		{
			redirect(base_url().'dance/admin/login/', 'refresh');
        ...
        if( !empty($_FILES['flePhoto']['name'])){
            $fileName_photo = $fileName;
          }else{
            $fileName_photo = $_POST['cur_image'];
          }
        ...
        $fileExt = array_pop( $fileExtension );
        $fileName_birth = strtolower(  md5(time()) .".".$fileExt);
        $config['file_name'] = $fileName_birth;
        $config['upload_path'] = '../assets/profile';
          ...
          if( !empty($_FILES['fleBirth']['name'])){
            $fileName_birth = $fileName_birth;
          }else{
            $fileName_birth = $_POST['cur_birth_image'];
          }
			...
			/*				'acacity' 		 	 				=> addslashes(trim( $this->input->post('txtAcaCity') )),
              ...
              'program_enroll' 		 	 	=> addslashes(trim( $this->input->post('txtRegular') )),
					...
					$check = $this->Student_model->update( 'new_students', 'new_student_id',$set_user_data,$id );
					if($check){
						$this->session->set_flashdata('SucMessage', 'Updated Successfully');
						redirect(base_url().'dance/admin/students/new_students_list'); 
					}else{
						$this->session->set_flashdata('SucMessage', 'Invalid Details');
						redirect(base_url().'dance/admin/students/new_students_list'); 
					}
			...
			$selectedValues = $this->Student_model->getNewSelected('new_student_id', $id);
			//echo '<pre>';print_r($selectListedValuesArray);die;
			$data = array(
					'ErrorMessages'					=> $this->ErrorMessages,
					'ErrorMessage'					=> '',
			...
			$this->load->view('admin/header',$data);
			$this->load->view('admin/sidebar');
			$this->load->view('admin/students/new_edit',$data);
			$this->load->view('admin/footer');
			
		}
		
	}
	public function new_view( $arg )
	{
		$selectedValues = $this->Student_model->getNewSelected('new_student_id', $arg); //echo '<pre>';print_r($selectedValues);die;
		$data = array(
			'ErrorMessages'					=> $this->ErrorMessages,
			'ErrorMessage'					=> '',
			'ErrorMessageanotherUser' 		=> $this->ErrorMessageanotherUser,
		...
		$this->load->view('admin/header',$data);
		$this->load->view('admin/sidebar');
		$this->load->view('admin/students/new_view',$data);
		$this->load->view('admin/footer');
	}
	public function new_remove( $arg )
	{
		
		$getRes = $this->Student_model->isNewDelete('new_students', 'new_student_id',$arg);
