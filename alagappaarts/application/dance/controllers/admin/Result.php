...
class Result extends CI_Controller {
 ...
 function __construct()
 {
   parent::__construct();
     ...
     $this->load->model('admin/Result_model','',TRUE);
 ...
 if($this->session->userdata('admin_logged_in') == False)
{
	...
	$this->load->view('admin/quiz/result_list_all',$data);
...
$user_id=$logged_in['id'];
 ...
 $logged_in=$this->session->userdata('admin_logged_in');
   ...
   $value=array();
    ...
    if($userid=="1"){  
   $data['result'] = $this->Result_model->result_view($id,$userid);
   //echo '<pre>1->';print_r($data['result']);exit;
 ...
 $data['percentile'] = $this->Result_model->get_percentile($quid, $data['result']->uid, $data['result']->score);
   ...
   $this->load->view('admin/quiz/quiz_header',$data);
   $this->load->view('admin/quiz/result_view',$data);
  	$this->load->view('admin/quiz/quiz_footer',$data);
 }
 ...
 function remove_result($rid)
 {
	...
	//$as = $this->Result_model->remove_result($rid);
	if($this->session->userdata['su']=="1" && $this->session->userdata['admin_logged_in']=="1"){
	$this->Result_model->remove_result($rid);
	redirect('dance/admin/result/index');
	}else{
 ...
 function view_answer($id)
 {
   $data['result'] = $this->Result_model->result_view($id);
	...
	$this->load->view('admin/quiz/quiz_header',$data);
   $this->load->view('admin/quiz/view_answer',$data);
  	$this->load->view('admin/quiz/quiz_footer',$data);
 }
...
if($this->db->update('essay_qsn', $data)){
   $this->db->where("rid",$r_id);
	$old_scr =$this->db->get("quiz_result");
         
	$old_re =$old_scr->row_array();
       $tot_opn = $old_re['score'] + $opn;
       
       
	$this->db->query("update quiz_result set score='".$tot_opn."' where rid='$r_id'");
	
	$this->db->where("quiz_result.rid",$r_id);
	$this->db->join("quiz","quiz.course_id=quiz_result.quid");
	...
	$perct=($score/($correct_marks['0']*100))*100;
	}
