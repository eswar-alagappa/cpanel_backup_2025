...
class Master extends CI_Controller {
	...
	public function settings( $type=null, $id=null)
	{
		...
		if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				$our_admin_password = $this->input->post('our_admin_password');
				...
				if( isset($password) && !empty($password)){
					...
					$this->form_validation->set_rules('global_password', 'Global Password', 'trim|required|min_length[8]|max_length[20]');
				}
				if( isset($our_admin_password) && !empty($our_admin_password)){
					$this->form_validation->set_rules('our_admin_password', 'OUR Admin Password', 'trim|required|min_length[8]|max_length[20]');
				}
				if($this->form_validation->run() == FALSE)
				{
					...
					$update_settings_data = array(
						...
						'our_admin_password'	=> ((isset($our_admin_password) && !empty($our_admin_password)) ? $this->encode(addslashes(trim( $this->input->post('our_admin_password') ))) : $selectedValues->our_admin_password ),
			...
			';
			$this->common_mail($user_data,$content,'APAA dance Super Admin Change Password',false);
						
						redirect(base_url().'dance/admin/login/signout'); 
					}else{
						$this->session->set_flashdata('SucMessage', 'Invalid Details');
						redirect(base_url().'dance/admin/master/settings'); 
					}
				}
		}
	...
	{	
		...
		if( isset($type) && !empty($type) && $type=='add')
		{
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				$this->form_validation->set_rules('mail_content', 'Mail Content', 'trim|required|min_length[10]');
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					$set_program_data = array(						
							...
							'mail_content' 		 	 				=> addslashes(trim( $this->input->post('mail_content') )),
					...
					if( isset($check) && !empty($check)){
						...
						redirect(base_url().'dance/admin/master/programs'); 
					}else{
						$this->session->set_flashdata('SucMessage', 'Invalid Details');
						redirect(base_url().'dance/admin/master/programs'); 
					}
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($check) && !empty($check)){
						...
						//if( isset($another_check) && !empty($another_check)){
							$this->session->set_flashdata('SucMessage', 'Successfully created');
							redirect(base_url().'dance/admin/master/courses'); 
						//}
					}else{
						$this->session->set_flashdata('SucMessage', 'Invalid Details');
						redirect(base_url().'dance/admin/master/courses'); 
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($check) && !empty($check)){
						...
						if( isset($another_check) && !empty($another_check)){
							$this->session->set_flashdata('SucMessage', 'Successfully created');
							redirect(base_url().'dance/admin/master/fees'); 
						}
					}else{
						$this->session->set_flashdata('SucMessage', 'Invalid Details');
						redirect(base_url().'dance/admin/master/fees'); 
	...
	public function status( $process, $arg )
	{
		if(isset($process) && !empty($process) && isset($arg) &&!empty($arg))
		{
			...
			if($getRes==true){
				...
				redirect(base_url().'dance/admin/master/'.$process); 
			}else{
				$this->session->set_flashdata('SucMessage', 'Invalid Details');
				redirect(base_url().'dance/admin/master/'.$process); 
			}
