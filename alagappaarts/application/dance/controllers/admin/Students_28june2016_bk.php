				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if($check){
						...
						if( isset($pass->is_publish_mail) && $pass->is_publish_mail == 0 ){ 
							...
							$pass = ((isset($pass->password) && !empty($pass->password)) ? $this->decode($pass->password) : '');
							$content = '
	Greetings from APAA! Your request for registering as APAA Dance Student has been successfully completed. The APAA customer support team has verified your Requested Profile. See below Your Username and Password <br>Username : '.$uname.'<br>Password : '.$pass.'';
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && !empty($id) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($check) && !empty($check))
					{
						...
						$content = 'APAA has assigned Exam You <br>'.$examDate.'Kindly Co-operate with us. Wish you All the Best';
