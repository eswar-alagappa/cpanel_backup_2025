	...
	public function sample_quiz_verify()
	{
		...
		// check if user have sufficient credit
			...
			$qidarrArray=$query->result_array();
				...
				shuffle($multipleQuest);shuffle($fillinQuest);shuffle($essayQuest);
				$multipleQuest1 = array_slice($multipleQuest, 0,40);
				$fillinQuest1 	= array_slice($fillinQuest, 0,20);
				$matchQuest 	= array_slice($questionArray3, 0,count($questionArray3));
				$essayQuest1	= array_slice($essayQuest, 0,6);
				shuffle($multipleQuest1);shuffle($fillinQuest1);shuffle($matchQuest);shuffle($essayQuest1);
	
				
				$qidarr = array_merge($multipleQuest1,$fillinQuest1,$matchQuest,$essayQuest1);
		...
		// check if user have sufficient credit
			...
			$qidarrArray=$query->result_array();
				...
				$essayQnLimit = ((isset($program_id) && !empty($program_id) && $program_id==1) ? 6 :
								 (isset($program_id) && !empty($program_id) && $program_id==3) ? 8 :
		...
		public function checkAlreadyEssayQn( $user_id, $rid,$qid)
		{
			...
			function quiz_submit($id)
			{
				...
				if( isset($_SESSION['session_rid']) && !empty($_SESSION['session_rid']))
				{
					...
					{						
						$this->session->set_flashdata('ErrMessage', 'Due To Internet connection problem your Exam Session Failed. Please Contact APAA Support team.');
					...
					if($essay_question >="1")
					{
						$q_result="2";
                                      ...
                                      $a_id=  $qr['assign_exam_id']; 
                                     
                                       
                                       $query = $this->db->query("select * from assign_exam where id ='$a_id' ");
					$ar = $query->row_array();
                                         $c_id =  $ar['course_id']; 
                                        if($ar['program_id'] == '4'){
