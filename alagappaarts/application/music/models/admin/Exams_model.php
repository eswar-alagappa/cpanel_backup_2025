...
function __construct()
	...
	public function getJoinList($table, $jointable, $joinField)
	{
			$this->db->select('*');
			$this->db->from($table.' t');
			$this->db->join($jointable.' jt','jt.'.$joinField.' = t.'.$joinField,'left');
	...
	public function multi_save($table,$data)
	{
	...
	public function multi_save_with_loop( $table,$table2,$post)
	{
		$cnt = count($post['match_question']);
		for( $i=0;$i<$cnt;$i++)
		{
			$set_question_data = array(						
					'question_type_id' 		 	 		=> addslashes(trim( $post['question_type'] )),
					'question' 		 	 				=> addslashes(trim( $post['match_question'][$i] )),
					'answer' 		 	 				=> addslashes(trim( $post['match_answer'][$i] )),							
					'status'							=> 1,
			 ...
			 $this->db->insert($table, $set_question_data);
			 $insertIdArray[] = $this->db->insert_id();
			
		}
		
		foreach($insertIdArray as $insert){
		$pcnt = count($post['course']);
			for($j=0;$j<$pcnt;$j++){
				$multiDataInsert1[] = array(
					'question_id' => $insert,
					'course_id' => $post['course'][$j]
				);
			}
			
		}
		
		$insertId = $this->db->insert_batch($table2, $multiDataInsert1);
				
		return ((isset($insertId) && !empty($insertId)) ? $insertId : false);
	}
