...
function __construct()
 ...
 // to search
  ...
  $query = $this -> db -> query("SELECT * FROM  `quiz_result` JOIN users ON quiz_result.uid = users.user_id JOIN user_profiles as up ON up.user_id=users.user_id JOIN quiz ON quiz_result.quid = quiz.course_id where $search_type like '%$search%' $condi and quiz_result.institute_id = '$institute_id' and quiz_result.quid!=0 order by quiz_result.rid desc limit $limit, $nor");

 
 }else{
	 
   $query = $this -> db -> query("SELECT * FROM  `quiz_result` JOIN users ON quiz_result.uid = users.user_id JOIN user_profiles as up ON up.user_id=users.user_id JOIN quiz ON quiz_result.quid = quiz.course_id $condi  and quiz_result.institute_id = '$institute_id' and quiz_result.quid!=0 order by quiz_result.rid desc limit $limit, $nor");
