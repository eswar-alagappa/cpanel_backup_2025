...
function __construct()
	...
	/*public function getList($table,$program_id,$center_id,$status)
	{
			...
			$this->db->join('user_program','user_program.user_id = users.user_id');
...
public function PaymentLista( $user_id,$program_id,$type)
	{
	...
	public function PaymentList( $user_id,$program_id,$type,$musictype)
	{
	...
	public function getSelected_List( $table, $selected_field, $id, $type )
	{
		if( !empty($table) && !empty($selected_field) && !empty($id) )
		{
			$this->db->select('*');
