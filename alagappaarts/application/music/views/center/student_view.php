...
<li><label>Address:  <?php echo $personal->address ?></label>
  ...
  <?php echo $personal->centerName ?></label>
     ...
     <fieldset class="w100 paymentDetail">
      ...
      <ul> <li class="button"><a class="backBtn" href="<?php echo base_url()?>music/center/students">Back</a>        </li> </ul>
      </div>
...
$(document).ready(function(){
        var pgm = $("select#selectProgram").val();
	    var userId = "<?php echo $user_id ?>";

		callAjax(pgm.split('-'),userId);
	...
	function callAjax(pgm,userId)
{

		$.ajax({
					url : "<?php echo base_url().'music/student/ajaxPayroll' ?>",
					type:"POST",
					data:{"program_id":pgm[0],'user_id':userId,'music_type':pgm[1]},
					dataType:"json",
