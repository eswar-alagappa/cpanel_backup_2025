...
window.location="<?php echo site_url();?>/music/admin/qbank/index/0/"+cid;
}
	</script>
...
<form method="post" action="<?php echo site_url('music/admin/quiz/edit_quiz/'.$result->quid);?>">
                                ...
                                <div class="col-lg-8">
										...
										<div class="form-group">
                                            <label>Course</label>
												...
												<option value="<?php echo $value->course_id; ?>" <?php echo ((isset($result->course_id) && !empty($result->course_id) && $result->course_id == $value->course_id) ? 'selected' : '') ?> > <?php echo $value->course_code; ?></option>
												<?php }} ?></select>
	                                      ...
	                                      <div class="form-group">
	                                       ...
	                                       <div class="form-group">
                                            <label>Start Time ( YYYY/MM/DD HH:MM:SS  )</label>
		                                          <input type='text' name='test_start_time'  id="start_datepicker" class="form-control" value="<?php echo date('Y/m/d',$result->start_time);?>">  
												  <?php echo form_error('test_start_time'); ?>
                                         </div>

		                                      <div class="form-group">
                                            <label>End Time ( eg. 2014/10/31 23:59:59 )</label>
		                                          <input type='text' name='test_end_time'  id="end_datepicker" class="form-control" 
												  value="<?php echo date('Y/m/d',$result->end_time);?>">  
									  ...
									  <div class="form-group">
	                                       ...
	                                       <!--<div class="form-group">
                                            ...
                                            <label>Correct answer score</label>
		                                          <input type='text' name='correct_answer_score' class="form-control" 
												  value="<?php echo ((isset($post_set['correct_answer_score']) && !empty($post_set['correct_answer_score'])) ? $post_set['correct_answer_score'] : 
												  ((isset($result->correct_score) && !empty($result->correct_score)) ? 
												  $result->correct_score : '1'));//echo $result->correct_score;?>" > 
												  <?php echo form_error('correct_answer_score'); ?>
                                         </div>
	                                       <div class="form-group">
                                            <label>Incorrect answer score</label>
		                                          <input type='text' name='incorrect_answer_score' class="form-control"  value="<?php echo ((isset($post_set['incorrect_answer_score']) && !empty($post_set['incorrect_answer_score'])) ? $post_set['incorrect_answer_score'] : 
												  ((isset($result->incorrect_score) && !empty($result->incorrect_score)) ? $result->incorrect_score : '0'));?>" >
...
if($result->qselect == "0"){
?><br> <br> 
<a href="javascript:questionselection('<?php echo $result->quid;?>','<?php echo $result->quiz_name;?>','0','0','<?php echo $result->course_id;?>');"  class="btn btn-success" >Add Questions Manually </a><br> <br> 
<?php
...
<td data-th="Action"><a href="<?php echo site_url('music/admin/qbank/edit_question/'.$row->qid.'/'.$row->q_type );?>" class="btn btn-info btn-xs"  target="edit_question">Edit</a>
<a href="<?php echo site_url('music/admin/quiz/remove_question_quiz/'.$result->quid.'/'.$row->qid );?>"  class="btn btn-danger btn-xs">Remove from Quiz</a>
<?php if($key!="0"){ ?>
...
questionselection('<?php echo $result->quid;?>','<?php echo $result->quiz_name;?>','0','1','<?php echo $result->course_id;?>');
...
window.location="<?php echo site_url('music/admin/quiz/up_question');?>/"+global_quid+"/"+global_qid+"/"+npos;
}else{
var npos=parseInt(pos)-parseInt(global_opos);
window.location="<?php echo site_url('music/admin/quiz/down_question');?>/"+global_quid+"/"+global_qid+"/"+npos;
}
}
</script>
