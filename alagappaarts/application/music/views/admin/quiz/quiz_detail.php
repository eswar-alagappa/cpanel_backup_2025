...
<script type="text/javascript" src="<?php echo base_url();?>/js/webcamjs/webcam.js"></script>
	...
	function gototest(){
	window.location='<?php echo site_url('music/admin/quiz/access_test/'.$result->quid);?>';
	}
	</script>
...
<script>
	...
	if(document.getElementById('agree').checked==true){
		<?php if($this->config->item('webcam_plugin') == true && $result->camera_req == '1'){ ?>void(take_snapshot());upload_photo();<?php }else{ ?>window.location='<?php echo site_url('music/admin/quiz/access_test/'.$result->quid);?>';<?php } ?>
		}else{
