...
<form method="post" action="<?php echo site_url('music/admin/result/get_report');?>">
...
<div class="searchbox" id="searchbox">
 <form method="post" action="<?php echo site_url('music/admin/result/index');?>">
...
<td data-th="Quiz Name"><?php 
$Qname = explode(' ', $row->quiz_name);
echo $Qname[count($Qname)-2].' '.$Qname[count($Qname)-1];
//echo $row->quiz_name;?></td>
...
<?php $query = $this->db->query("SELECT * FROM error_event WHERE rid='".$row->rid."'");
$error_event  = $query->result_array();
if(count($error_event)  > 0){
    $error = $error_event[0]['error_report'];
}else{
    $error = '-';
}
?>
<td data-th="Reason"><?php echo $error ?> </td>
<td data-th="Action">
<a href="<?php echo site_url('music/admin/result/view/'.$row->rid.'/'.$row->course_id.'/'.$row->uid);?>" title="Result View" alt="Result View" class="fa fa-envelope-o"></a>  
<!--<a href="<?php echo site_url('music/admin/result/view/'.$row->rid.'/'.$row->course_id.'/'.$row->uid);?>" title="Result Announce" alt="Result Announce" class="fafa-envelope"></a>--> 
<?php
if($logged_in['su']=="1" && $logged_in['id']=="1"){
?>
<a href="<?php echo site_url('music/admin/result/remove_result/'.$row->rid);?>" class="btn btn-danger btn-xs">Remove</a>
<?php
}
?>
...
if($logged_in=="1"){
?>
<a href="<?php echo site_url('music/admin/result/index/'.$back);?>"   class="btn btn-primary">Back</a>
&nbsp;&nbsp;
<?php
 $next=$limit+($this->config->item('number_of_rows'));  ?>

<a href="<?php echo site_url('music/admin/result/index/'.$next);?>"   class="btn btn-primary">Next</a>
<?php
}else{
?>
