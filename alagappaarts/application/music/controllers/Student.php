...
class Student extends CI_Controller {
	...
	public function change_password()
	{
		if($this->session->userdata('site_logged_in') == false){
			redirect(base_url().'music', 'refresh');
		}
		$user_id = $this->session->userdata('site_user_id');
		$post_set 				= $_POST;
			...
			if($this->form_validation->run() == FALSE )
			{
				...
				$set_user_data = array(
				 ...
				 if( isset($check) && !empty($check)){
					...
					redirect(base_url().'music/student/profile'); 
				}else{
					$this->session->set_flashdata('SucMessage', 'Invalid Details');
					redirect(base_url().'music/student/change_password'); 
				}
				...
				if($this->form_validation->run() == FALSE )
				{
					...
					$this->ErrorMessages		 		= validation_errors();
						...
						if( isset($check) && !empty($check)){
							...
							$this->session->set_flashdata('SucMessage', 'Invalid Details');
							redirect(base_url().'music/student/edit_profile'); 
						}
			...
			if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($check) && !empty($check)){
						$this->session->set_flashdata('SucMessage', 'Successfully created');
						redirect(base_url().'music/student/feedback'); 
					}
	...
	public function captcha_refresh(){ 
	   ...
	   public function Admin_mail($user_data,$content,$subject,$cc)
	   {
			...
			$this->email->initialize($config);
			$this->email->from( $user_data['studentemail'], $user_data['firstname'].' '.$user_data['lastname'] );
			$this->email->to($user_data['email'] );
			if( isset($cc) && !empty($cc) && $cc==true){
				$this->email->cc(CUSTOMER_EMAIL);
			}
			...
			$message=$this->load->view('Admin_mail',$data,TRUE);
			//echo $message;die;
			$this->email->message($message);
			$resultMail = $this->email->send();
			return ((isset($resultMail) && !empty($resultMail) && $resultMail==1) ? true : false);
	...
	public function registration()
	{
		...
		{
			redirect(base_url().'music/student/index', 'refresh');
		}
		...
		$captcha_error			= '';
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Register') ) {
				...
				$this->form_validation->set_rules('exp_music', 'Experience of bharatanatyam', 'trim|required|min_length[1]');
				...
				if($this->form_validation->run() == FALSE )
				{
					...
					$set_userprofile_data = array(						
							...
							'music_experience' 		 	 		=> addslashes(trim( $this->input->post('exp_music') )),
...
$content .= "<tr style='background: #eee;'><td><strong>Name:</strong> </td><td>" .addslashes(trim( $this->input->post('first_name') )) . "</td></tr>";
$content .= "<tr><td><strong>Last Name:</strong> </td><td>" .addslashes(trim( $this->input->post('last_name') )) . "</td></tr>";
$content .= "<tr><td><strong>Program:</strong> </td><td>" .$_POST['hdn_program'] . "</td></tr>";

$content .= "<tr ><td><strong>Music Type:</strong> </td><td>" . $_POST['hdn_musictype'] . "</td></tr>";

$content .= "<tr><td><strong>Age:</strong> </td><td>" .addslashes(trim( $this->input->post('age') )) . "</td></tr>";
$content .= "<tr><td><strong>DOB</strong> </td><td>" .addslashes(trim( date('Y-m-d',strtotime($this->input->post('dob'))) )). "</td></tr>";




$content .= "<tr><td><strong>Gender:</strong> </td><td>" .addslashes(trim( $this->input->post('gender') )). "</td></tr>";
$content .= "<tr><td><strong>Alternate Phone No:</strong> </td><td>" .addslashes(trim( $this->input->post('other_contact') )) . "</td></tr>";
$content .= "<tr><td><strong>Special accomplishments:</strong> </td><td>" .addslashes(trim( $this->input->post('txtSpecqualification') )) . "</td></tr>";
$content .= "<tr><td><strong>Other Relevane Info:</strong> </td><td>" . addslashes(trim( $this->input->post('txtotherinfo') )). "</td></tr>";

$content .= "<tr><td><strong>Center:</strong> </td><td>" .$_POST['hdn_center']. "</td></tr>";


$content .= "<tr><td><strong>Address</strong> </td><td>" .addslashes(trim( $this->input->post('address') )). "</td></tr>";

$content .= "<tr><td><strong>City:</strong> </td><td>" .addslashes(trim( $this->input->post('city') )) . "</td></tr>";
$content .= "<tr><td><strong>State:</strong> </td><td>" . addslashes(trim( $this->input->post('state') )) . "</td></tr>";
$content .= "<tr ><td><strong>Country:</strong> </td><td>" . addslashes(trim( $this->input->post('country') )) . "</td></tr>";
$content .= "<tr><td><strong>Zip</strong> </td><td>" .addslashes(trim( $this->input->post('zip') )). "</td></tr>";
$content .= "<tr><td><strong>Mobile:</strong> </td><td>" .addslashes(trim( $this->input->post('contact') )) . "</td></tr>";

$content .= "<tr><td><strong>Email:</strong> </td><td>" .addslashes(trim( $this->input->post('email') )) . "</td></tr>";
$content .= "<tr><td><strong>Experience in Music:</strong> </td><td>" . addslashes(trim( $this->input->post('exp_music') )) . "</td></tr>";
$content .= "<tr><td><strong>Name of your Guru</strong> </td><td>" . addslashes(trim( $this->input->post('name_of_guru') )). "</td></tr>";


$content .= "<tr><td><strong>Your Guru Located at:</strong> </td><td>" .addslashes(trim( $this->input->post('txtLoca') )) . "</td></tr>";
...
$subject = 'New Student Registeration Notification';
					//	$content = 'Greetings from APAA! Your request for registering as APAA Dance Student has been successfully completed. The APAA customer support team will get back to you shortly.';
					//	$subject = 'Greetings from APAA!';
						...
						$user_data = array(
							...
							'email'		=> "customersupport@alagappaarts.com",
								'studentemail'		=>  addslashes(trim( $this->input->post('email') ))
						);
						//$this->Admin_mail($user_data,$content,$subject,false);
						
						
						
						
						$this->session->set_flashdata('SucMessage', 'Thanks for your Registration. Please wait for Admin approval');
						redirect(base_url().'music/student/registration'); 
					}else{
			...
			$centers = $this->Student_model->getListactiveState('center_academy');
			 ...
			 /*$values = array(
                ...
                'img_path' => '../assets/images/',
                'img_url' =>  base_url() .'assets/images/',
                'font_path'  => base_url() . 'system_music/fonts/texb.ttf',
                'img_width' => '150',
                'img_height' => 50,
                'expiration' => 3600
               );*/
	...
	public function sample_exam_process()
	{
		...
		if( isset($data['resultstatus']) && !empty($data['resultstatus']) && $data['resultstatus'] != ""){
				...
				redirect('music/student/sample_exam_process/', 'refresh');
				} 
		...
		{ 
			if( isset($course) && !empty($course) && $course != 'online_exam_process' )
			{
				...
				else
				{
					...
					redirect('music/student/online_exam/', 'refresh');
				}
			}
			elseif( empty($course))
			{
		...
		if( isset($_SESSION['qid']) && !empty($_SESSION['qid']))
		{
			...
			{  
				...
				{				
						...
						{ 
							redirect('music/student/exam_process/', 'refresh');
						}
...
//                                               if($data['assigned_question'] == '9'){
//                                                   redirect(base_url('music/student/index'));  
//                                               }
                                                //
						// get time information
						$data['time_info']=$this->Student_model->get_time_info($rid);//echo '<pre>';print_r($data['time_info']);die;
      ...
      $query = $this->db->query("select * from assign_exam  where id='$assign_exam_id'");
     $e_check = $query->row_array(); 
     
     if($e_check['questions_assigned'] == ''){
     $this->session->set_flashdata('ErrMessage', 'Question is not assigned');    
     redirect(base_url('music/student/index'));    
     }
	...
	public function online_exam_process()
	{
		...
		{	
			...
			if(!empty($_SESSION['qid']))
			{
				...
				redirect(base_url().'music/student', 'refresh');
			...
			foreach ($menu['parent_menus'][$parent] as $k=>$menu_id) {
				if (!isset($menu['parent_menus'][$menu_id])) {
					...
					$isActive = ((!empty($urlString) && $urlString == $menu['menus'][$menu_id]['link']) ? 'current_page_item active' : '');
							...
							$html .= "<li class='menu-item menu-item-type-post_type menu-item-object-page ".$isActive."'><a class='has-parent' href='" . base_url().'music/'.$menu['menus'][$menu_id]['link'] . "'>" . stripslashes($menu['menus'][$menu_id]['name']) . "</a></li>";	
						}
					}else{
						$urlString = uri_string();
					$isActive = (( empty($urlString) && $menu['menus'][$menu_id]['name']=='Home') ? 'current_page_item active': (!empty($urlString) && $urlString == $menu['menus'][$menu_id]['link']) ? 'current_page_item active' : '');
						$html .= "<li class='menu-item menu-item-type-post_type menu-item-object-page ".$isActive."'><a href='" . base_url().'music/'.$menu['menus'][$menu_id]['link'] . "'>" . stripslashes($menu['menus'][$menu_id]['name']) . "</a></li>";
					}
					$i++;
				} 
				if (isset($menu['parent_menus'][$menu_id])) { $urlString = uri_string();
					...
					$html .= '<li class=" '.$isActive.'"><a class="dropdown-toggle" data-toggle="dropdown" href=' . base_url().'music/'.$menu['menus'][$menu_id]['link'] . '>' . $menu['menus'][$menu_id]['name'] . '<span class="caret"></span></a>';
