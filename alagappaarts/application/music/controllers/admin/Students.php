...
<p><a target="_blank" href="'.$login.'">'.$login.'</a></p>
	...
	{	
		/*$query = mysql_query('select * from quiz_result');
		$listData = mysql_fetch_assoc($query);
		echo '<pre>';print_r($listData);
		die;*/
		if($this->session->userdata('admin_logged_in') == False)
		{
			redirect(base_url().'music/admin/login/', 'refresh');
	...
	{	
		...
		$post_set = $_POST;
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				$this->form_validation->set_rules('music_type', 'Music Type', 'trim|required|min_length[1]');
				$this->form_validation->set_rules('center_id', 'Center', 'trim|required|min_length[1]');
				$this->form_validation->set_rules('exp_music', 'Experience of Music', 'trim|required|min_length[1]');
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					$set_userprofile_data = array(						
							...
							'music_type'         							=> addslashes(trim( $this->input->post('music_type') )),
					...
					if( isset($check) && !empty($check)){
						...
						if( isset( $_POST['program_id'])){
							...
							$stream = ((isset($_POST['stream']) && !empty($_POST['stream'])) ? $_POST['stream'] : '');
...
fees payment, etc.</p>
			...
			$music_type = array(1=>'Instrumental Music','2'=>'Vocal Music');
			$sreatm = array();
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if($check){
						...
						if( isset($pass->is_publish_mail) && $pass->is_publish_mail == 0 ){ 
							...
							$pass = ((isset($pass->password) && !empty($pass->password)) ? $this->decode($pass->password) : '');
							$content = '
	Greetings from APAA! Your request for registering as APAA Music Student has been successfully completed. The APAA customer support team has verified your Requested Profile. See below Your Username and Password <br>Username : '.$uname.'<br>Password : '.$pass.'';
	...
	public function result_exam($id=null)
	{
		...
		if( isset($id) && !empty($id))
		{
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($check) && !empty($check))
					{
						...
						redirect(base_url().'music/admin/students/index'); 
								...
								redirect(base_url().'music/admin/students/result_exam/'.$id);
							}
			...
			$defaultMusicType = $getDefaultPgmArray[0]->music_type;
			$music_type = array(1=>'Instrumental Music','2'=>'Vocal Music');
	...
	public function course_completion($arg)
	{
		...
		if(isset($arg) && !empty($arg))
		{
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($_POST['is_eligible']) && !empty($_POST['is_eligible']) && $_POST['is_eligible']==1){
						...
						if( isset($check) && !empty($check))
						{
							...
							$this->session->set_flashdata('SucMessage', 'Invalid Details');
							redirect(base_url().'music/admin/students/course_completion/'.$arg);
						}
					}else{
						$this->session->set_flashdata('SucMessage', 'Invalid Details');
						redirect(base_url().'music/admin/students/course_completion/'.$arg);
			...
			$ExamScore   	= $this->Student_model->ExamScore($arg,$program_id);
			   	...
			   	$paymentList= $this->Student_model->PaymentList($arg,$program_id,'',$defaultMusicType);
				...
				if($this->form_validation->run() == FALSE)
				{
					...
					{ 
						...
						if( isset($quizResult) && !empty($quizResult) && count($quizResult) >0) 
						{
							...
							redirect(base_url().'music/admin/students/assign_exam/'.$id);
						}else{
                                                    
                                                    $query = $this->db->query("SELECT * FROM con_qes ");  
           $quest_che = $query->result_array();              
                   if($quest_che[0]['status'] == '0'){ 
//                                                  print_r($_POST);
                                                    if($program_id == '4'){
                                                $quest_type = array('0','2','4','5');  
                                                     $quest_type_text = array('Multiple Choice -single answers','Fill in the Blank','Essay','Matching');  
                                                     
                                                foreach ($_POST['course_code'] as $value) {
                                                          
                                                $query = $this->db->query("SELECT qids_static FROM quiz WHERE course_id = '".$value."' ");  
                                                $quest_check = $query->result_array(); 
                                                    echo    $quest_check[0]['qids_static'];
//                                             exit();
                                              if(count($quest_check) > 0){
                                                   $query = $this->db->query("SELECT q_type FROM qbank WHERE course_id = '".$value."'  AND FIND_IN_SET(qid,  '".$quest_check[0]['qids_static']."') ");  
                                                $quest_check = $query->result_array(); 
                                                foreach ($quest_type as $key =>  $q_type) {
                                                  if(!in_array($q_type, array_unique(array_column($quest_check, 'q_type')))){
                                                        $query = $this->db->query("SELECT course_code FROM courses WHERE course_id = '".$value."' ");  
                                                $course_id = $query->result_array(); 
                                                $_SESSION['Errorpop']  =  'Please add the'.$quest_type_text[$key].' Question in '.$course_id[0]['course_code'];
						  redirect(base_url().'music/admin/students/assign_exam/'.$id);
                                                  } 
                                                }
                                                 
                                              }else{
                                                  $_SESSION['Errorpop']  = 'Please add the Question';
					redirect(base_url().'music/admin/students/assign_exam/'.$id);
                                              }
                                                     ...
                                                     $quest_type = array('0','2','4','5','6');  
                                                     $quest_type_text = array('Multiple Choice -single answers','Fill in the Blank','Essay','Matching','True / False');  
...
//                                                print_r($quest_check);
                                                foreach ($quest_type as $key =>  $q_type) {
                                                  if(!in_array($q_type, array_unique(array_column($quest_check, 'q_type')))){
                                                        $query = $this->db->query("SELECT course_code FROM courses WHERE course_id = '".$value."' ");  
                                                $course_id = $query->result_array(); 
//                                                print_r(array_unique(array_column($quest_check, 'q_type')));
//                                                echo $q_type;
//                                                exit();
                                                    $_SESSION['Errorpop']  =  'Please add the'.$quest_type_text[$key].' Question in '.$course_id[0]['course_code'];
						  redirect(base_url().'music/admin/students/assign_exam/'.$id);
...
//                                                    exit();
							...
							if( isset($check) && !empty($check))
							{
								...
								$examDate .= 'This is to notify you in advance the date and time of the '.rtrim($pgmCode,',').' online
													...
													the examination anytime during the scheduled dates of '.rtrim($dates,',').' . Please take online sample test to get familiar with the
													format.';
			...
			$paymentList   	= $this->Student_model->PaymentList($id,$defaultProgram_id,$defaultMusicType,'');
			
			$selectedPgm = $selectedCourse = array();
			if( isset($_POST['program_id']) && !empty($_POST['program_id']))
			{
				$selectedCourse = $this->Student_model->getSelected_List('courses','program_id','music_type',$_POST['program_id'],$_POST['music_type'],'');
			}
	...
	{
		if( isset($_POST['type']) && !empty($_POST['type']) && isset($_POST['program_id']) && !empty($_POST['program_id']) && isset($_POST['user_id']) && !empty($_POST['user_id'])  ){
			$program_id = trim($_POST['program_id']);
			$table = trim($_POST['type']);
			$id = trim($_POST['user_id']);
			$music_type	 = trim($_POST['music_type']);
			$ExamScore   	= $this->Student_model->ExamScore($id,$program_id);
			$paymentList   	= $this->Student_model->PaymentList($id,$program_id,'',$music_type);
	...
	public function ajaxPgm()
	{
		if( isset($_POST['type']) && !empty($_POST['type']) && isset($_POST['field_id']) && !empty($_POST['field_id']) && isset($_POST['field_val']) && !empty($_POST['field_val']) ){
			...
			$paymentList   	= $this->Student_model->PaymentLista($id,$field_val,$music_type,'');
			$course = $this->Student_model->getSelected_List($table,$field_id,$field_val,$music_type);
	...
	public function reset_password( $arg )
	{
		...
		if(isset($arg) && !empty($arg))
		{
				...
				{
						$this->session->set_flashdata('SucMessage1','Center Member Is not Active');
						redirect(base_url().'music/admin/students/index'); 
				}
		}
