...
class Centers extends CI_Controller {
	...
	public function reset($arg)
	{
		...
		if(isset($arg) && !empty($arg))
		{
				...
				$user_data = $this->Center_model->getSelected1('center_academy_id',$arg);
				...
				if( $user_data->status ==1)
				{
					...
					$this->email->to( $user_data->cemail );
					//$this->email->cc('sk@sanjaytechnologies.net');
					//$this->email->bcc();
					$this->email->subject('APAA Center Credentials');
					...
					$this->session->set_flashdata('SucMessage', 'Successfully Send Mail');
						redirect(base_url().'music/admin/centers/index'); 
				}
	...
	{	
		...
		$post_set = $_POST;
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				$this->form_validation->set_rules('academy_city', 'City', 'trim|required|min_length[2]');	
				//$this->form_validation->set_rules('academy_website', 'Grace Period Year', 'trim|required|max_length[1]');				
				$this->form_validation->set_rules('academy_state', 'State', 'trim|required|min_length[2]');
				$this->form_validation->set_rules('academy_phone', 'Contact', 'trim|required|min_length[10]');
				$this->form_validation->set_rules('academy_country', 'Country', 'trim|required|min_length[2]');
				...
				$this->form_validation->set_rules('director_state', 'State', 'trim|required|min_length[2]');
				$this->form_validation->set_rules('director_email', 'Email', 'trim|required|valid_email');
				$this->form_validation->set_rules('director_country', 'Country', 'trim|required|min_length[2]');
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($center_user_id) && !empty($center_user_id) && isset($check) && !empty($check)){
						...
						$pass = $this->Center_model->getPassword($uname);
						$pass = ((isset($pass->password) && !empty($pass->password)) ? $this->decode($pass->password) : '');
						$content = '
Greetings from APAA! See below Your Username and Password <br>Username : '.$uname.'<br>Password : '.$pass.'';
