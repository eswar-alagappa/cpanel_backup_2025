...
class Home extends CI_Controller {
	...
	{	
		...
		{
			redirect(base_url().'music/center/index', 'refresh');
		}
		...
		if (isset($menu['parent_menus'][$parent])) {
			...
			foreach ($menu['parent_menus'][$parent] as $k=>$menu_id) {
				if (!isset($menu['parent_menus'][$menu_id])) {
					if(!empty($parent)){ 
						//$html .= (($i==1) ? '<li class="current-menu-item"><div class="caret-1"></div></li>' :'<li class="divider"></li>');
					...
					$isActive = ((!empty($urlString) && $urlString == $menu['menus'][$menu_id]['link']) ? 'current-menu-item active' : '');
						...
						$urlString = uri_string();
					$isActive = (( empty($urlString) && $menu['menus'][$menu_id]['name']=='Home') ? 'current-menu-item active': (!empty($urlString) && $urlString == $menu['menus'][$menu_id]['link']) ? 'current-menu-item active' : '');
				...
				if (isset($menu['parent_menus'][$menu_id])) { $urlString = uri_string();  
					$isActive = ((!empty($urlString) && !empty($parentMenu) && ($menu['menus'][$menu_id]['menu_id'] == $parentMenu)) ? ' current-menu-item active' : '');
				...
				if($this->form_validation->run() 		== FALSE)
				{
					...
					$this->ErrorMessages				= validation_errors();
						...
						if( !empty($loginChk))
						{
							...
							$this->email->send(); 
							redirect(base_url().'music/registration');
						}else{
							$this->session->set_flashdata('SucMessage', 'Incorrect Email and Contact');
							redirect(base_url().'music/forgetpassword');
						}
					...
					//echo '<pre>';print_r($loginChk);die;
						...
						if( !empty($loginChk->user_role_id) && $loginChk->user_role_id ==2){ 
							...
							$user_data = array('site_user_name' 		=> $loginChk->username,
								...
								$this->session->set_userdata($user_data);		  
							redirect(base_url().'music/center/index');
						}else{ 
	...
	public function contactus()
	{
		...
		if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['sendbutton']) && $_POST['sendbutton']=='Submit') ) {
			...
			if($this->form_validation->run() == FALSE )
			{
				...
				redirect(base_url().'music/contact-us'); 
	...
	public function pages( $arg )
	{
		...
		if( isset($arg1) && !empty($arg1) && isset( $arg2 ) && !empty($arg2) && ($arg2!='apaa-programs' && $arg2 !='fee-structure' && $arg2 !='faq' && $arg2 !='photo-gallery' && $arg2 !='video-gallery' ))
		{
			...
			//echo 'b';die; 
			$this->faq($arg1);
		}else if( isset($arg1) && !empty($arg1) && isset( $arg2 ) && !empty($arg2) && (($arg1=='academics') && ($arg2=='apaa-programs' || $arg2 =='fee-structure') && isset($arg3) && !empty($arg3) ) ){ //echo $arg3;die;
			//echo 'c';die;
			$this->pgm_course($arg2,$arg3);
	...
	public function pgm_course( $type, $course_type )
	{
		if( isset($type) && !empty($type) && $type == 'apaa-programs' && is_numeric($course_type))
		{ //echo 'a';die;
			$ctype = ((isset($course_type) && !empty($course_type) && $course_type==1) ? 1 :
					((isset($course_type) && !empty($course_type) && $course_type==2) ? 2 : 1));
			$pgmCourse = $this->Home_model->getProgramCourse($ctype);
		...
		{ //echo 'b';die;
			...
			$pgmFeesList = $this->Home_model->getProgramFees($ctype);
