...
class Center extends CI_Controller {
			...
			if($this->form_validation->run() == FALSE )
			{
				...
				$set_user_data = array(
				 ...
				 if( isset($check) && !empty($check)){
					...
					redirect(base_url().'music/center/profile'); 
				}else{
					$this->session->set_flashdata('SucMessage', 'Invalid Details');
					redirect(base_url().'music/center/change_password'); 
				}
				...
				if($this->form_validation->run() == FALSE )
				{
					...
					$this->ErrorMessages		 		= validation_errors();
						...
						if( isset($check) && !empty($check)){
							...
							$this->session->set_flashdata('SucMessage', 'Invalid Details');
							redirect(base_url().'music/center/edit_profile'); 
						}
	...
	public function exam_result()
	{
		...
		/* pagination */
		//pagination settings
        $config['base_url'] = site_url('music/center/exam_result');
	...
	public function exam_schedule()
	{
		...
		/* pagination */
		//pagination settings
        $config['base_url'] = site_url('music/center/exam_schedule');
	...
	{	
		...
		/* pagination */
		//pagination settings
        $config['base_url'] = site_url('music/center/students');
	...
	{	
		...
		/* pagination */
		//pagination settings
        $config['base_url'] = site_url('music/center/payments');
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Register') ) {
				...
				if($this->form_validation->run() == FALSE )
				{
					...
					$set_directory_data = array(						
							...
							'experience_music' 		 				=> addslashes(trim( $this->input->post('exp_music') )),
					...
					if( isset($check) && !empty($check)){
						...
						$content = '
Greetings from APAA! Your request for registering as APAA Music Center has been successfully completed. The APAA customer support team will get back to you shortly.';
						...
						redirect(base_url().'music/center/registration'); 
					}else{
		...
		$this->load->view('center/center_footer');
	}
