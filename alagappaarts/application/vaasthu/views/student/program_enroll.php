     ...
     <fieldset class="w100 paymentDetail">
      ...
      <li>
        <label>Graduation Date :
           <?php echo ((isset($programList[0]->completion_date) && !empty($programList[0]->completion_date)) ? date('d-M-Y',strtotime($programList[0]->completion_date)) : '-'); ?>  
         </label>  
        </li>
        <li>
        <label>Grade:
				...
				$.ajax({
					...
					success: function(data) {
						...
						for(var k=0;k<data.length;k++){
							$('.pgmDetail').find('.first li:nth-child(1) label').html('Student ID : '+data[k].username);
							$('.pgmDetail').find('.first li:nth-child(2) label').html('Centre Name :'+ data[k].centerName);
							
							$('.pgmDetail').find('.second li:nth-child(1) label').html('Date of joining : '+data[k].enrollment_date);
							$('.pgmDetail').find('.second li:nth-child(2) label').html('Program fee :  $ '+data[k].Total_fee);
							...
							tableData += "<tr><td>"+data[k].amount+"</td><td>"+data[k].payment_option+"</td><td>"+data[k].payment_mode+"</td><td>"+data[k].paid_on+"</td><td>"+data[k].check_no+"</td><td>"+data[k].paymentStatus+"</td><td>"+data[k].comments+"</td><tr>";
							
							$('.courseDetail').find('ul li.CCstatus').html('Graduated :'+data[k].graduation_status);
							$('.courseDetail').find('ul li.CC-Comments').html('Comments :'+data[k].graduation_status_comments);
							
							$('.courseDetail').find('.graduate li:nth-child(1) label').html('Graduation Date : '+data[k].completion_date);
							$('.courseDetail').find('.graduate li:nth-child(2) label').html('Grade :  '+data[k].grade);
						}
