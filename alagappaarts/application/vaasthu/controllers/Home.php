...
class Home extends CI_Controller {
	...
	{	
		...
		{
			redirect(base_url().'vaasthu/student/index', 'refresh');
		}
		if($this->session->userdata('site_logged_in') == true && $this->session->userdata('site_user_role') == '3' )
		{
			redirect(base_url().'vaasthu/center/index', 'refresh');
		}
				...
				if($this->form_validation->run() 		== FALSE)
				{
					...
					$this->ErrorMessages				= validation_errors();
						...
						if( !empty($loginChk))
						{
							...
							$this->email->send();
							redirect(base_url().'vaasthu/registration');
						}else{
							$this->session->set_flashdata('SucMessage', 'Incorrect Email and Contact');
							redirect(base_url().'vaasthu/forgetpassword');
						}
	...
	public function registration( $type )
	{
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				$this->form_validation->set_rules('special_accomplishment','Special Accomplishment','trim|required|min_length[3]');
				...
				if($this->form_validation->run() == FALSE )
				{
					...
					$set_userprofile_data = array(						
							...
							'special_accomplishment' 		 	 			=> addslashes(trim( $this->input->post('special_accomplishment') )),
							'created_by'									=> 'User',
							'updated_by'									=> 'User',
					...
					if( isset($check) && !empty($check)){
						...
						$this->session->set_flashdata('SucMessage', 'Thanks for your Registration.');
						redirect(base_url().'vaasthu/registration'); 
					}else{
			...
			$programs = $this->Home_model->getListactiveState('programs');
			$centers = array('1'=>'Vaasthu Science World Research Centre');
			
			$data = array(
				'ErrorMessages'					=> $this->ErrorMessages,
				'ErrorMessage'					=> '',
				'ErrorMessageanotherUser' 		=> $this->ErrorMessageanotherUser,
				'SiteTitle' 					=> $this->SiteMainTitle.'- Register',
				'SiteMainTitle' 				=> $this->SiteMainTitle,
				'post_set'						=> $post_set,
				'programs'						=> $programs,
				'centers'						=> $centers,
				...
				if($this->form_validation->run() 		== FALSE)
				{
					...
					//echo '<pre>';print_r($loginChk);die;
						...
						if( !empty($loginChk->user_role_id) && $loginChk->user_role_id ==2){ 
							...
							redirect(base_url().'vaasthu/student/index');
							...
							$user_data = array('site_user_name' 		=> $loginChk->username,
								...
								$this->session->set_userdata($user_data);		  
							redirect(base_url().'vaasthu/center/index');
						}else{ 
							redirect(base_url().'vaasthu/admin/login/signout');
						}
	...
	public function contactus()
	{
		...
		if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['sendbutton']) && $_POST['sendbutton']=='Submit') ) {
			...
			if($this->form_validation->run() == FALSE )
			{
				...
				redirect(base_url().'vaasthu/contact-us'); 
	...
	public function getLeftbanners()
	{
		...
		if( isset($urlParams) && !empty($urlParams))
		{
			...
			$getMenuList = $this->Home_model->menuList($urlParams);// echo '<pre>';print_r($getMenuList);die;
			return $getMenuList;
		}
