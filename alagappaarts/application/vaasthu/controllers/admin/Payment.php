...
class Payment extends CI_Controller {
	...
	{	
		...
		$payments = $this->Payment_model->getJoinRecordList();	
		
		if( isset($payments) && !empty($payments)){
			foreach($payments as $payment){
				$paymentDet = $this->Payment_model->getPayment($payment->user_id,$payment->program_id);
				if( isset($paymentDet) && !empty($paymentDet)){
					for( $i=0;$i< count($paymentDet); $i++){
						if( $paymentDet[$i]->program_id &&  $paymentDet[$i]->student_id){
							
							$totalPayAmount[$paymentDet[$i]->program_id][$paymentDet[$i]->student_id][] = $paymentDet[$i]->amount;
						}
					}
				}
			}
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($check) && !empty($check)){
						...
						redirect(base_url().'vaasthu/admin/payment/index'); 
					}
