			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($check) && !empty($check)){
						...
						redirect(base_url().'vaasthu/admin/settings/menu'); 
					}else{
		...
		if( isset($type) && !empty($type) && $type=='add')
		{
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($check) && !empty($check)){
						$this->session->set_flashdata('SucMessage', 'Successfully created');
						redirect(base_url().'vaasthu/admin/settings/pages'); 
					}else{
						$this->session->set_flashdata('SucMessage', 'Invalid Details');
						redirect(base_url().'vaasthu/admin/settings/pages'); 
					}
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && !empty($id) && (!empty($_POST['update']) && $_POST['update']=='Update') ) {
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					$set_page_data = array(						
							...
							//'created_at'							=> date('Y-m-d H:i:s'),
							'updated_at'							=> date('Y-m-d H:i:s'),
					);//echo '<pre>post->';print_r($_POST);echo '<pre>';print_r($set_page_data);die;
			...
			if(($this->input->server('REQUEST_METHOD') == 'POST') && !empty($id) && (!empty($_POST['update']) && $_POST['update']=='Update')) {
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if($check){
						...
						redirect(base_url().'vaasthu/admin/settings/feedback'); 
					}else{
						$this->session->set_flashdata('SucMessage', 'Invalid Details');
						redirect(base_url().'vaasthu/admin/settings/feedback/reply/'.$id); 
					}
				}
			}
		...
		if( isset($type) && !empty($type) && $type=='add')
		{
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Submit') ) {
				...
				if($this->form_validation->run() 			== FALSE)
				{
					...
					if( isset($check) && !empty($check)){
						$this->session->set_flashdata('SucMessage', 'Successfully created');
						redirect(base_url().'vaasthu/admin/settings/faq'); 
					}else{
						$this->session->set_flashdata('SucMessage', 'Invalid Details');
						redirect(base_url().'vaasthu/admin/settings/faq'); 
					}
	...
	public function status( $process, $arg )
	{
		if(isset($process) && !empty($process) && isset($arg) &&!empty($arg))
		{
			...
			if($getRes==true){
				...
				redirect(base_url().'vaasthu/admin/settings/'.$process); 
			}else{
				$this->session->set_flashdata('SucMessage', 'Invalid Details');
				redirect(base_url().'vaasthu/admin/settings/'.$process); 
			}
