...
class Center extends CI_Controller {
	...
	{ 
			...
			$data = $this->Home_model->getStudentDetails($center_academy_id);
	...
	public function change_password()
	{
		if($this->session->userdata('site_logged_in') == false){
			redirect(base_url().'vaasthu', 'refresh');
		}
		$user_id = $this->session->userdata('site_user_id');
		$post_set 				= $_POST;
			...
			if($this->form_validation->run() == FALSE )
			{
				...
				$set_user_data = array(
				 ...
				 if( isset($check) && !empty($check)){
					...
					redirect(base_url().'vaasthu/center/profile'); 
				}else{
					$this->session->set_flashdata('SucMessage', 'Invalid Details');
					redirect(base_url().'vaasthu/center/change_password'); 
				}
				...
				if($this->form_validation->run() == FALSE )
				{
					...
					$this->ErrorMessages		 		= validation_errors();
						...
						if( isset($check) && !empty($check)){
							...
							$this->session->set_flashdata('SucMessage', 'Invalid Details');
							redirect(base_url().'vaasthu/center/edit_profile'); 
						}
			...
			foreach ($menu['parent_menus'][$parent] as $k=>$menu_id) {
				if (!isset($menu['parent_menus'][$menu_id])) {
					...
					$isActive = ((!empty($urlString) && $urlString == $menu['menus'][$menu_id]['link']) ? 'current_page_item active' : '');
							...
							$html .= "<li class='menu-item menu-item-type-post_type menu-item-object-page ".$isActive."'><a class='has-parent' href='" . base_url().'vaasthu/'.$menu['menus'][$menu_id]['link'] . "'>" . stripslashes($menu['menus'][$menu_id]['name']) . "</a></li>";	
						}
					}else{
						$urlString = uri_string();
					$isActive = (( empty($urlString) && $menu['menus'][$menu_id]['name']=='Home') ? 'current_page_item active': (!empty($urlString) && $urlString == $menu['menus'][$menu_id]['link']) ? 'current_page_item active' : '');
						$html .= "<li class='menu-item menu-item-type-post_type menu-item-object-page ".$isActive."'><a href='" . base_url().'vaasthu/'.$menu['menus'][$menu_id]['link'] . "'>" . stripslashes($menu['menus'][$menu_id]['name']) . "</a></li>";
					}
					$i++;
				} 
				if (isset($menu['parent_menus'][$menu_id])) { $urlString = uri_string();
					...
					$html .= '<li class=" '.$isActive.'"><a class="dropdown-toggle" data-toggle="dropdown" href=' . base_url().'vaasthu/'.$menu['menus'][$menu_id]['link'] . '>' . $menu['menus'][$menu_id]['name'] . '<span class="caret"></span></a>';
	...
	public function exam_result()
	{
		...
		/* pagination */
		//pagination settings
        $config['base_url'] = site_url('vaasthu/center/exam_result');
	...
	public function exam_schedule()
	{
		...
		/* pagination */
		//pagination settings
        $config['base_url'] = site_url('vaasthu/center/exam_schedule');
	...
	public function student_view($arg)
	{
		...
		if( isset($arg) && !empty($arg))
		{
			...
			$paymentListArray 		= $this->Student_model->PaymentList($arg);
			$examTaken 				= $this->Center_model->examTaken($arg);
			$personal 	= $this->Center_model->studentPersonal($arg);
			//echo '<pre>';print_r($paymentListArray);die;
			$data = array(
				'personal' 			=> $personal,
				'programList' 		=> $programList,
				'pgmEnroll' 		=> $program_enroll,
				'paymentList' 		=> $paymentListArray[0],
				'examTaken'			=> $examTaken[0],
				'user_id'			=> $arg
			);
			$this->load->view('center/header');
			//$this->setHeader();
	...
	{	
		...
		/* pagination */
		//pagination settings
        $config['base_url'] = site_url('vaasthu/center/students');
	...
	{	
		...
		/* pagination */
		//pagination settings
        $config['base_url'] = site_url('vaasthu/center/payments');
	...
	public function registration()
	{
		...
		$captcha_error			= '';
			...
			if (($this->input->server('REQUEST_METHOD') == 'POST') && empty($arg) && (!empty($_POST['submit']) && $_POST['submit']=='Register') ) {
				...
				$this->form_validation->set_rules('academy_city', 'Academy City', 'trim|required|min_length[3]');
				$this->form_validation->set_rules('academy_state', 'Academy State', 'trim|required|min_length[3]');
				$this->form_validation->set_rules('academy_phone', 'Academy Contact', 'trim|required|min_length[10]');
				$this->form_validation->set_rules('academy_country', 'Academy Country', 'trim|required|min_length[3]|max_length[100]');
				$this->form_validation->set_rules('academy_zip', 'Academy Zipcode', 'trim|required|min_length[3]');
				...
				if($this->form_validation->run() == FALSE )
				{
					...
					if( isset($check) && !empty($check)){
						...
						redirect(base_url().'vaasthu/center/registration'); 
					}else{
			...
			$centers = $this->Center_model->getListactiveState('center_academy');
			 ...
			 /*$values = array(
                ...
                'font_path'  => base_url() . 'system_vaasthu/fonts/texb.ttf',
                'img_width' => '150',
                'img_height' => 50,
                'expiration' => 3600
               );*/
