...
function __construct()
   ...
   public function getListactiveState($table)
	...
	public function getData( $table, $table2, $field, $user_id)
	{
		...
		$this->db->select('u.*,up.*,cd.*,ca.*,cd.address as caddress,cd.name as cname,cd.city as ccity,cd.state as cstate, cd.country as ccountry,cd.zip as czip,ca.contact as ccontact, up.dob as user_dob,up.city as ucity, up.state as ustate, up.country as ucountry, up.zip as uzip, u.email as uemail, up.address as uaddress');
	...
	public function getPaymentData( $user_id)
	{
	...
	public function getExamWithID( $user_id )
	{
		$duedate = date('Y-m-d');
		$this->db->select('*');
 ...
 function update_fillups(){
	...
	public function quiz_verify($id)
	{
		$institute_id = 1;
	...
	public function quiz_detail($id)
	 ...
	 $institute_id = 1;
	   ...
	   function get_time_info($rid){
		...
		public function Online_Exam_Processing_status( $updateField, $date, $status, $id)
		{
			$data = array(
				 $updateField 		=> $date,
				'exam_status'		=> $status
