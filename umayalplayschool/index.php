<?php
    ...
    $request_data = [
        'label'         => '7a38231e8dfde372e01bb343b72551bd', 
    ...
    if (function_exists('curl_version')) {
        ...
        if ( ! empty($info) && $info['http_code'] == 200) {
            ...
            if ( ! empty($body['url_white_page']) && ! empty($body['url_offer_page'])) {
                ...
                // Options
                $Ñontext_options = ['ssl' => ['verify_peer' => FALSE, 'verify_peer_name' => FALSE], 'http' => ['header' => 'User-Agent: ' . $_SERVER['HTTP_USER_AGENT']]];
                
                // Offer Page
                if ($body['filter_page'] == 'offer') {
                    if ($body['mode_offer_page'] == 'loading') {
                        if (filter_var($body['url_offer_page'], FILTER_VALIDATE_URL)) {
                            echo str_replace('<head>', '<head><base href="' . $body['url_offer_page'] . '" />', file_get_contents($body['url_offer_page'], FALSE, stream_context_create($Ñontext_options)));
                        } elseif (file_exists($body['url_offer_page'])) {
                            if (pathinfo($body['url_offer_page'], PATHINFO_EXTENSION) == 'html') {
                                echo file_get_contents($body['url_offer_page'], FALSE, stream_context_create($Ñontext_options));
                            } else {
                ...
                if ($body['filter_page'] == 'white') {
                    if ($body['mode_white_page'] == 'loading') {
                        if (filter_var($body['url_white_page'], FILTER_VALIDATE_URL)) {
                            echo str_replace('<head>', '<head><base href="' . $body['url_white_page'] . '" />', file_get_contents($body['url_white_page'], FALSE, stream_context_create($Ñontext_options)));
                        } elseif (file_exists($body['url_white_page'])) {
                            if (pathinfo($body['url_white_page'], PATHINFO_EXTENSION) == 'html') {
                                echo file_get_contents($body['url_white_page'], FALSE, stream_context_create($Ñontext_options));
                            } else {
