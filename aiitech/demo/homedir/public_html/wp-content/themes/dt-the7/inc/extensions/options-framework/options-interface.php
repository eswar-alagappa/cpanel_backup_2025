...
function optionsframework_interface( $options, $cur_page_id ) {
	...
	foreach ( $options as $value ) {
		...
		case 'slider':
			...
			$output .= '<input type="text" class="of-slider-value"' . $str . ' name="' . esc_attr($option_name . '[' . $value['id'] . ']') . '" value="' . $slider_opts['value'] . '"/>';
		...
		case 'pages_list':
			...
			$output .= The7_Option_Field_Pages_List::html( $field_name, $field_id, $val );
			break;
		...
		case 'icons_picker':
			...
			$icons       = isset( $value['default_icons'] ) ? array( _x( 'Default', 'theme options', 'the7mk2' ) => (array) $value['default_icons'] ) : array();
